...
Biometric credentials are prevalent in modern authentication. Historically, they were used in signatures and handprints on royal documents and contracts~\cite{biometrichistory}, as they rely on unique physical characteristics of the user made them unforgeable. However, in the early days of the Internet, biometric credntials were not applicable and therefore, credentials were typically \emph{atomic}, a credential was either presented correctly or not according to a perfect match criterion. Examples include passwords~\cite{password1979}, one-time password (OTP)~\cite{otp2017} and physical devices~\cite{FIDO2021}. While such credentials are simple to verify, they are vulnerable to various attacks, such as password leaks, account recovery attacks, and device theft.

A research conducted in the U.S reveals statistics behind password vulnerabilities: 59\% of users in the U.S. reported leaks and 22\% frequently forget their passwords~\cite{password_leak}. Consequently, biometric credentials are increasingly adopted, present in modern user devices, e.g, where access to most smartphones relies on face or fingerprint verification. The biometric sensors obtain a \emph{sample}, calculate a similarity score between that sample and a \emph{reference} stored in advance, and output \emph{accept} if the score is above a set \emph{match threshold} or \emph{reject} otherwise.

Previous work~$(\S\ref{sec:2})$ has conducted extensive analysis of biometric credentials in isolation, primarily focusing on improving accuracy, aiming to reduce the false acceptance~(\FAR) and false rejection~(\FRR) rates~\cite{Anil1997,prabhakar2003}. Sarkar et al.~\cite{sarkar2020} introduce the Equal Error Rate (\EER, where~$\FAR = \FRR$), a heuristic threshold of biometric sensors. While these works widely cover biometric credentials, none of them analyze their integration into an authentication mechanism. Other works analyze authentication mechanisms as predicates of credential possession~\cite{maram2024,mouallem2024,Eyal2021,Abhishek2013}, where each credential is treated as atomic. But they neglect the potential due to the special properties of biometric credentials.

In this work, we study how to design mechanisms with biometric credentials. We analyze how biometric credentials influence the authentication mechanism's success with and without atomic credentials. Our goal is to optimally use biometric credentials in authentication mechanisms and analyze how the presence of atomic credentials influences the choice of the biometric credential \emph{match threshold}. First, we define the authentication problem~$(\S\ref{sec:3})$, which comprises an authentication mechanism and two players a \emph{user} and an \emph{attacker}. Both players are trying to satisfy the mechanism by providing a set of credentials. Each credential can be either presented by~\cite{Eyal2021}, (1)~only the user~(then we call it~\emph{safe}), (2)~neither player~(\emph{lost}), (3)~both players~(\emph{leaked}), or (4)~only the attacker~(\emph{stolen}). Each atomic credential has a probability of being in each of the four states. 

Biometric credentials are evaluated by a sensor that calculates a \emph{confidence score} from an obtained sample. If the score is above a \emph{match threshold}, the sensor outputs accept, otherwise reject. The genuine distribution~$f_U(s)$ denotes the score distribution induced by samples produced by the legitimate user, whereas the impostor distribution~$f_A(s)$ denotes the score distribution induced by samples produced by the attacker as shown in Figure~\ref{fig:intro-fig}. These distributions establish two error metrics: known as \emph{false acceptance rate~(\FAR)} and \emph{false rejection rate~(\FRR)}. 

We examine distribution functions that approximate a biometric credential score distribution. We characterize each player by their score distribution.
Intuitively, genuine users are expected to produce higher matching scores than attackers. Therefore, the user score distribution is expected to be shifted toward higher matching scores compared to the attacker distribution. Otherwise, the attacker would be more likely to produce scores that lead to successful authentication, rendering the biometric credential ineffective. To define the state probabilities of the biometric credential, we calculate the~\FAR and~\FRR metrics which are functions of the match threshold. An optimal mechanism has the maximum probability of success, which depend on the number of credentials and their state probabilities.

We seek to identify optimal mechanisms. Thus, we start by optimizing a single biometric credential mechanism~$(\S\ref{sec:4})$. To define the biometric credential state probabilities, we calculate~\FAR and~\FRR metrics for each biometric credential approximation. Then, we derive closed-form expressions for the mechanism's success to identify the optimal threshold of the biometric credential. We then symbolically calculate the optimal and~\EER thresholds and show that in general, the optimal threshold differs from the commonly used~\EER. To understand the potential improvement for deviating from the~\EER, we link the likelihood of an attacker producing higher matching scores to the gap between the maximum success and the success at~\EER. Our findings reveal that the more likely an attacker is to produce higher matching scores~$s$, and the more asymmetric the user and attacker distributions become, the greater the benefit of using the optimal threshold instead of~\EER.
...
Having characterized the optimal threshold of a single biometric credential mechanism, we extend our analysis to a more complex case that combines two credentials~$(\S\ref{sec:5})$. These may include two biometric credentials, as in airport or border-control systems, or one biometric and one atomic credential, as in cloud service authentication. For example, Google~\cite{google_passkeys_2023} assumes (1) Possession of a trusted device like a phone or computer (Atomic credential) and (2) Authentication via biometric credentials~\cite{mahfouz2025}, e.g, face or fingerprint. 

To evaluate a mechanism comprising two biometric credentials, we consider first and second order approximations and use a numerical optimization algorithm to find the optimal thresholds for each biometric credential. Our results show that the optimal thresholds of the biometric credentials in such a mechanism is not, in general, the same as the optimal thresholds if used individually. This difference arises as we optimzie each credential with respect to the entire mechanism, and not only with respect to the credential itself.
...
We then turn to analyze mechanisms that integrate a biometric and an atomic credential. Trying to identify the impact of an atomic credential on the optimal match threshold, we derive closed-form expressions for the mechanism's success probability. We symbolically calculate the optimal and~\EER thresholds and find that the optimum depends on the values of the atomic credential. These expressions reveal that the mechanism's success can primarily depend on either an atomic credential or a biometric credential, depending on the atomic credential probability values. For example, in an \emph{AND mechanism}, the mechanism grants access to a user if and only if all credentials are available to the user and at least one credential is unavailable to the attacker. The derived relation for this mechanism shows that when the atomic credential has a high probability of being safe, the mechanism’s success probability depends mainly on the atomic credential. In this case, even biometric samples that closely match the user reference have only a limited impact on the overall mechanism success probability; conversely, for other probability values of an atomic credential, success may depend more strongly on the biometric credential.

Motivated by this observation, we analyze whether the integration of a biometric credential with an atomic credential will always enhance mechanism's success probability~$(\S\ref{sec:6})$. First, we link the biometric credential to the improvement it has on the mechanism's success probability when it comprises an atomic credential. This link shows that when an atomic credential has zero \emph{loss} or \emph{leak} state probabilities, it is not worthwhile to integrate, as the atomic credential mechanism dominates the mechanism success probability. Then, we characterize the conditions under which integrating a biometric and an atomic credential improves the mechanism's success probability. 

We introduce a necessary and sufficient condition for an \emph{asymptotic separable} biometric credential: for every score above or below a match threshold, the error rates converge to zero. We prove that every \emph{asymptotically separable} biometric credential integrated with an atomic credential that has non-zero probabilities for the states of \emph{safe} and either \emph{loss} or \emph{leak}, improves the mechanism success probability. However, biometric credentials often exhibit a more common and weaker behavior~\cite{Slob2005,anil2004}. Thus, we introduce \emph{asymptotic dominance}: where the genuine distribution dominates the attacker distribution by a factor $c$ for every score above a specific threshold, or the attacker dominates the user by a factor $c$ for every score below a specific threshold. We show that this condition is sufficient for a biometric credential to improve the mechanism's success probability when integrated with an atomic credential.

We quantitatively demonstrate our theoretical analysis using public biometric data and a standard algorithm for fingerprint verification~\cite{sourceafis}~$(\S\ref{sec:7})$. We use a dataset of fingerprints from multiple users and evaluate matching scores by comparing samples of one user against those of other users. When calculating the matching score produced when different players attempt to authenticate as one another, we observe that the algorithm effectively extracts minutiae and the score is low. This results in a low~\EER value and a small overlapping area between the user and the attacker distributions. Thus, no significant improvement is observed when comparing the mechanism's success under the~\EER threshold and the optimal threshold. This indicates that the~\EER threshold may be sufficient and satisfactory when different users are trying to authenticate as one another.


However, since an attacker does not attempt to authenticate using their own biometric features, the relevant threat model involves attempts to physically impersonate a legitimate user. We conduct a spoofing attack evaluation using the LivDet database~\cite{LivDet2009,LivDet2011,LivDet2013,LivDet2015}, which includes both genuine fingerprint scans and corresponding spoofing attempts. These spoof samples produce matching scores that closely resemble those of the legitimate user. Integrating the biometric credential into a mechanism comprised of an atomic credential, reduces the mechanism’s failure probability by~30-70\% compared to the probability with the \EER threshold. Furthermore, the results show a~50\% reduction in the mechanism’s failure probability compared to a mechanism based solely on an atomic credential, demonstrating the benefit of integrating the biometric credential.

In summary~$(\S\ref{sec:8})$, our contributions are both theoretical and practical: (1)~Formalization of the mechanism's success function composed of biometric and atomic credentials; (2)~Symbolic analysis of the optimal threshold for biometric and atomic authentication mechanisms. (3)~Characterization of the conditions under which biometric credential integration enhances the mechanism's success; (4)~Proof that any biometric credential, under a specified condition, always enhances the mechanism's success. (5)~Empirical analysis using real biometric data. (6)~Direct implications suggesting biometric sensor vendors allow setting the sensors' thresholds for immediate security improvements.
...
The system comprises an authentication mechanism~$M$ and two players: a user $U$ and an attacker~$A$. Each player interacts with the mechanism~$M$ by \emph{presenting credentials}. We abstract away the concrete implementation of a credential and model only its presentation to the mechanism. Denote the set of credentials by
\begin{equation*}
    \mathcal{C}=\{c_1,c_2,\ldots,c_n\}
\end{equation*}
The system runs an execution, which describes the interaction between the players and mechanism until the mechanism makes a decision. In an execution, each player~$P$ may \emph{present} a credential~$c_i$ to the mechanism by calling a random function~$\textit{present}_P(i)$. The outcome of the~\emph{presentation function} determines whether the credential is available to the player or not, and the mechanism makes a decision based on this outcome. We define the availability of credentials as
\begin{definition}(Availability Vector)
    An \emph{availability vector} specifies, for each credential, whether a player successfully presented each credential or not. Denote by $\sigma^U$, \textit{and} $\sigma^A$ the availability vectors of the user and of the attacker, respectively.
\end{definition}

For a player~$P\in\{U,A\}$, the entry~$\sigma_i^P=1$ means that~$P$ successfully presented credential~$c_i$, and~$\sigma_i^P=0$ means that it is not. The outcome of the presentation function determines whether~$\sigma_i^P=1$ or $\sigma_i^P=0$. Denote the set of credentials available to player~$P$ by
\begin{equation*}
    C_P(\sigma)=\{c_i\in\mathcal{C}|\sigma_i^P=1\}.
\end{equation*}

Each credential can be either:~$\textit{Safe}$: Only available to the user, $\textit{Lost}$: Not available to either player, $\textit{Leaked}$: Available to both players, or~$\textit{Stolen}$: Only available to the attacker. A scenario~$\sigma$ is the vector of states of each credential in the mechanism. We denote by~$\sigma_i$ the state of credential~$c_i$ in scenario~$\sigma$. 
Each state corresponds to the outcome of the presentation functions for a user and an attacker. Specifically, the pair $(\textit{present}_U(i), \textit{present}_A(i))$ determines whether the credential is~\emph{Safe}, \emph{Loss}, \emph{Leak} or \emph{Stolen}. Therefore, the state probabilities are
\begin{equation*}
...
A credential may correspond to secret data, such as a password or one time password~(OTP); a physical object, such as a phone or security key. We model these credentials as atomic: the player either presents the credential correctly or not in a perfect match criterion. 
\begin{definition} (Atomic Credential)~If a credential is atomic, Denote by~$c^\star_{i}$ the correct credential stored in advance, then the presentation function~$\textit{present}_P(i)$ defined by 
\begin{equation*}
...
\begin{definition} (Biometric Credential)~If a credential is biometric, the presentation function~$\textit{present}_P(i)$ is defined by~$f_U(s)$,~$f_A(s)$ and a match threshold~$t$. For a player~$P\in\{U,A\}$, the presentation function draw random value for the score~$s$ from the distribution~$f_P(s)$, and outputs~1 if~$s\ge t$ and~0 otherwise.
    \begin{equation*}
    \textit{present}_P(i) = \begin{cases}
        1 &  s \ge t \\
        0 & \text{otherwise}
    ...
    \end{equation*}
\end{definition} 
The user and attacker distributions establish two error metrics:~\FAR and~\FRR both are functions of the match threshold~$t$. 
\begin{equation}
    \label{eqn:FAR}
...
\end{equation}
We use the~\FAR and~\FRR to find the probability of the biometric credential availability states. The user has access to the biometric cedential when the output of the biometric sensor is \emph{accept}, which occurs with probability~$1-\FRR(t)$. The attacker has access to the biometric credential when the output of the biometric sensor is \emph{accept}, which occurs with probability~$\FAR(t)$. Thus, the probabilities of the four states are
\begin{equation*}
...
\begin{itemize}
     \item~$C_P(\sigma)$ the set of credentials available to player~$P$ in scenario~$\sigma$ (perhaps an empty set, represented by~$\varnothing$).
\end{itemize}
...
The mechanism is successful in a scenario~$\sigma$ if it grants access to the user only. 
\begin{definition} (Mechanism Success) A mechanism is \emph{successful} in a scenario~$\sigma$ if and only if the authentication mechanism decision~$\mathrm{decide}_M(C_P(\sigma))$ equals~1 for the user only, that is
\begin{equation*}
    \mathrm{decide}_M(C_U(\sigma))=1 \wedge \mathrm{decide}_M(C_A(\sigma))=0.
\end{equation*}
\end{definition} 
...
P_{\text{Success}}(M)
&= \Pr\big[
\mathrm{decide}_M(C_U(\sigma))=1 \\
&\qquad\wedge\ 
\mathrm{decide}_M(C_A(\sigma))=0
\big].
\end{aligned}
\end{equation*}
For example, in an \emph{AND mechanism}~\cite{Eyal2021}, the mechanism grants access to a player if and only if all credentials are available to the player. In an \emph{OR mechanism}, the mechanism grants access to a player if at least one credential is available to the player.
...
(1)~$a_2 < u_1$~(Figure~\ref{fig:uniform_case1_pdf}): In this case, the attacker and user distributions are \emph{separated}. For a mechanism~$M$ with a single credential~$c_1$, the mechanism's success probability is simply the probability that the credential is safe:
\begin{equation}
...
We maintain the same approach as the case~(2) to calculate the probability of success. The~\FAR is one for any~$t \le a_1$ and zero for any~$t \ge a_2$, and the~\FRR is zero for any~$t \le u_1$ and one for any~$t \ge u_2$. Thus, in this case, we obtain 4 regions of the success probability function:
\begin{equation*}
P_{\text{success}}(t) = 
\begin{cases}
    0, & t \le a_1 \\
...
Thus, the maximum point is at~$t_{\text{Max}} = \frac{a_1 + u_2}{2}$ if:~$\frac{a_1+u_2}{2} \in [u_1, a_2]$ in case~(3), or~$\frac{a_1+u_2}{2} \in [a_1, a_2]$ in case~(2).
...
\end{equation*}
For case~(3), if the condition is not satisfied the maximum point is at the boundary of the interval, which is either~$t = u_1$ or~$t = a_2$. Thus
\begin{equation*}
P_{\text{success}}(t_{\text{opt}}) = \max\left\{\frac{u_1 - a_1}{a_2 - a_1}, \frac{u_2 - a_2}{u_2 - u_1}\right\}.  
\end{equation*}
For case~(2), if the condition is not satisfied the maximum point is at the boundary of the interval, which is either~$t = a_1$ or~$t = a_2$. Thus
\begin{equation*}
...
Figure~\ref{fig:FAR_FRR_uniform_1_cred} indicate that when the user and attacker distributions are symmetric, i.e., when the intervals of each distribution are equal:~${a_2 - a_1 = u_2 - u_1}$, the~\FAR equals the~\FRR.
