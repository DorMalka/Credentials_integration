...
\end{definition}
When a player~$P$ presents an atomic credential~$c_i$, the presentation function ouputs~1 with probability~$P_{\text{safe}} + P_{\text{leak}}$ when allocated to the user and 0 with probability~$P_{\text{leak}} + P_{\text{stolen}}$ when allocated to the attacker.
...
\begin{definition} (Biometric Credential) For a biometric credential, the user score distribution~$f_U(s)$, the attacker score distribution~$f_A(s)$, and the match threshold~$t$ define the presentation function~$\textit{present}_P(i)$. For each player~${P\in\{U,A\}}$, the allocation function~$\textit{allocate}_P(i)$ draws a random score~$s$ from the distribution~$f_P(s)$. Thus
    \begin{equation*}
...
\begin{array}{@{}l@{\qquad}l@{}}
     \dfrac{6(s-\alpha_1)(s-\alpha_2)}{(\alpha_1-\alpha_2)^3} & \alpha_1 \leq s \leq \alpha_2 \\
     0, & \text{otherwise}
\end{array}
\right.
&&
...
\left\{
\begin{array}{@{}l@{\qquad}l@{}}
     \dfrac{6(s-\beta_1)(s-\beta_2)}{(\beta_1-\beta_2)^3} & \beta_1 \leq s \leq \beta_2 \\
     0, & \text{otherwise}
\end{array}
\right.
&&
...
\frac{
\begin{aligned}
&2(\beta_2^3-t^3)-3(\beta_1+\beta_2)(\beta_2^2-t^2)\\
&\quad+6\beta_1\beta_2(\beta_2-t)
\end{aligned}
}{
(\beta_1-\beta_2)^3
},
...
\frac{
\begin{aligned}
&2(t^3-\alpha_1^3)-3(\alpha_1+\alpha_2)(t^2-\alpha_1^2)\\
&\quad+6\alpha_1\alpha_2(t-\alpha_1)
\end{aligned}
}{
(\alpha_1-\alpha_2)^3
}.
...
\frac{
\left(
a_2 P^{1}_{\text{safe}} - a_1 \big(P^{1}_{\text{safe}} + P^{1}_{\text{leak}}\big)
+ P^{1}_{\text{leak}}\, u_2
\right)^2
}{
...
\end{proof}
\noindent
We showed that~$\bar{M}$ achieves greater success probability than that of~$M$ the biometric credential is \emph{asymptotically separable} and the atomic credential has~$P^1_\text{safe} > 0$ and at least one of~$P^1_\text{leak},P^1_\text{loss} > 0$. We also presented that asymptotic separability is a necessary condition for~$\bar{M}$ achieving greater success probability than every atomic credential mechanism. We extend our proof to a mechanism that uses a biometric credential and~$k$ atomic credentials and show that the mechanism achieves greater success probability than that of a mechanism using only the atomic credentials. First, every authentication mechanism~$M$ can be represented as a sum of products, i.e., a disjunctive normal form~(DNF)~\cite{Mendelson1997}. Then, we define a \emph{consolidated credential} of~$k$ atomic credentials as if it were itself a single atomic credential.

\begin{definition}
Let~$M$ be an authentication mechanism over~$k$ atomic credentials. The \emph{consolidated credential} of~$M$ is the single abstract credential whose availability to the user and attacker is determined by the outputs of~$M$ on their respective availability vectors~$\sigma^U,\sigma^A\in\{0,1\}^k$. The fault probabilities of the consolidated credential are thus
\begin{align*}
P^M_{\text{safe}}
&=
...
\begin{proposition}\label{thm:suf_nec_cond_for_improve_k_atm}
    Let~$\{c_1,c_2,\ldots,c_k\}$ be a set of~$k$ independent atomic credentials,~$c_{k+1}$ be a biometric credential whose user distribution~$f_U(t)$ and attacker distribution~$f_A(t)$ are asymptotically separable and~$M$ be a mechanism using~$\{c_1,c_2,\ldots,c_k\}$ whose consolidated credential has~$P^M_\text{safe} > 0$ and at least one of~$P^M_\text{leak},P^M_\text{loss} > 0$. Then, there exists a mechanism~$\bar{M}$ using~$\{c_1,c_2,\ldots,c_k,c_{k+1}\}$ such that
    \begin{equation*}
        P_{\mathrm{success}}^{\bar{M}} > P_{\mathrm{success}}^{M}
    \end{equation*}
\end{proposition}
\begin{proof}
    Let~$M$ be a mechanism using~$\{c_1,c_2,\ldots,c_k\}$. The mechanism~$M$ induces a reduction of the~$k$ credentials into a single consolidated credential. The consolidated credential of~$M$ is a single atomic credential with state probabilities~$P^M_\text{safe}>0$, and at least one of~$P^M_\text{leak}$, or~$P^M_\text{loss} > 0$. By Proposition~\ref{thm:suf_nec_cond_for_improve}, there exists a mechanism~$\bar{M}$ using the consolidated credential and the biometric credential that achieves greater success probability than that of the consolidated credential mechanism. Since the consolidated credential is equivalent to the original mechanism~$M$, it follows that there exists a mechanism~$\bar{M}$ using~$\{c_1,c_2,\ldots,c_k,c_{k+1}\}$ that achieves greater success probability than that of~$M$.
\end{proof}
\begin{proposition}
    Let~$c_{k+1}$ be a biometric credential with user distribution~$f_U(t)$ and attacker distribution~$f_A(t)$. Suppose that for every set of~$k$ independent atomic credentials~$\{c_1,\ldots,c_k\}$ and every mechanism~$M$ over these credentials whose consolidated credential has~$P^M_\text{safe} > 0$ and at least one of~$P^M_\text{leak},P^M_\text{loss} > 0$, there exists a mechanism~$\bar{M}$ using~$\{c_1,\ldots,c_k,c_{k+1}\}$ such that
    \begin{equation*}
    P^{\bar{M}}_{\text{success}}(t) > P^{M}_{\text{success}}.
    \end{equation*}
...
\end{proposition} 
\begin{proof}
    The mechanism~$M$ induces a reduction of the~$k$ credentials into a single consolidated credential. The consolidated credential of~$M$ is a single atomic credential with state probabilities~$P^M_\text{safe}>0$, and at least one of~$P^M_\text{leak},P^M_\text{loss} > 0$. Then,~$\bar{M}$ is a mechanism using the consolidated credential of~$M$ and the biometric credential. By Proposition~\ref{thm:nec_cond_for_improve}, the biometric credential must be asymptotically separable.
\end{proof}
...
\section{Empirical Evaluation}\label{sec:7}
\noindent
We now empirically evaluate different mechanisms that use an atomic credential and a biometric credential under two attack scenarios. In each scenario, we compare the mechanism's success probability when the threshold is set to the~\EER and when it is optimized. We first simulate a baseline attack in which an attacker attempts to authenticate using their own fingerprints~$(\S\ref{sec:7.1})$. We show that although optimizing the threshold increases the mechanism's success probability, the improvement is limited. We then present a more sophisticated attack in which an attacker creates a fake fingerprint using spoofing techniques~$(\S\ref{sec:7.2})$ and show that the improvement becomes substantially larger. To reflect real-world authentication systems, we evaluate spoofing attack when multiple attempts are allowed~$(\S\ref{sec:7.3})$. We show that in such cases, the threshold should be optimized with respect to the maximum number of allowed attempts for siginificant improvement in the mechanism's success probability.

\subsection{Baseline Attack Scenario}\label{sec:7.1}
\noindent
To simulate a biometric credential, we use the public real-world~\textit{FVC2004} dataset of fingerprints, developed for the third international Fingerprint Verification Competition to compare fingerprint verification algorithms~\cite{fvc2004}. This dataset contains multiple fingerprint scans of each user, acquired using different types of biometric sensors. We divide each user's scans in the dataset into a reference set, simulating the reference scans stored in advance, and a probe set, simulating the scans obtained by the sensor. To score the fingerprint scan images, we use the standard fingerprint verification algorithm~\textit{SourceAFIS}~\cite{sourceafis}. This algorithm compares the relative positions and orientations of fingerprint ridge endings and bifurcations across two images. We evaluate all~10 users who appear in~$\textit{FVC2004}$ dataset whose fingerprints obtained by the same sensor. For each user, we randomly select three fingerprint scan images for the reference set, while the probe set comprises the remaining images in the dataset. We compute a similarity score for each probe scan by running the algorithm against each of the three reference images, and choose the maximum score as the probe's final score~\cite{hong1998,Cappelli2006,max_score2009}. The genuine comparisons are all comparisons between the user's probe images and the user's reference images. By contrast, the imposter comparisons are all comparisons between the users' reference images and the scans of the other users in the database.

Calculating the mechanism's success probability requires the biometric credential's fault probabilities, which are functions of the~\FAR and~\FRR. We therefore follow the evaluation procedure used by Jain et al.~\cite{Anil1997}, Cappelli et al.~\cite{Cappelli2006} and Slobodan et al.~\cite{Slob2005}. We produce histograms of the genuine and imposter comparisons, as shown in Figure~\ref{fig:sourceafis_histogram}. Then, we calculate the~\FAR and~\FRR as functions of the threshold~$t$, as shown in Figure~\ref{fig:sourceafis_far_frr}.

Before comparing different mechanisms, we validate that the algorithm's performance is representative of the state-of-the-art fingerprint verification algorithms. The algorithm we use achieves an~\EER of approximately~2\%, similar to the best results reported in the~\textit{FVC2004} competition~\cite{fvc2004}, and to those achieved by fingerprint systems used in industry and academia when evaluating different users~\cite{Maio2002,Cappelli2006}. We calculate the biometric credential fault probabilities~(Equation~\ref{eqn:safe_prob}) and the success probability of a mechanism that uses a biometric credential only~(Equation~\ref{eqn:safesuccess}). We find that the optimal threshold differs from the~\EER point and achieves a higher success probability, as shown in Figure~\ref{fig:bio_only_sourceafis_reg_scoring}. 

\newcommand{\sourceafisplotspanel}{%
\begin{figure*}[!t]
\centering
...
\addlegendimage{fill=red, mark=none}
\addlegendentry{Impostor}
\end{axis}
\end{tikzpicture}
\medskip
\pgfplotslegendfromname{histogramlegend}
...
% Empirical FAR: discrete step function
\addplot[
    thick,
    red,
...
\addlegendentry{Empirical \FAR}

% Empirical FRR: discrete step function
\addplot[
    thick,
    blue,
    const plot mark left,
...
\addlegendentry{Empirical \FRR}

% Uniform FAR: continuous theoretical curve
\addplot[
    thick,
    red,
    dashed,
...
table[x=T, y=FAR_uniform] {figs/fig_different_users_sourceafsi/sourceafis_far_frr_data.txt};
\addlegendentry{Uniform \FAR}

% Uniform FRR: continuous theoretical curve
\addplot[
    thick,
    blue,
    dashed,
...
table[x=T, y=FRR_uniform] {figs/fig_different_users_sourceafsi/sourceafis_far_frr_data.txt};
\addlegendentry{Uniform \FRR}

% Quadratic FAR: continuous theoretical curve
\addplot[
    thick,
    red,
    densely dotted,
...
table[x=T, y=FAR_quadratic] {figs/fig_different_users_sourceafsi/sourceafis_far_frr_data.txt};
\addlegendentry{Quadratic \FAR}

% Quadratic FRR: continuous theoretical curve
\addplot[
    thick,
    blue,
    densely dotted,
...
table[x=T, y=FRR_quadratic] {figs/fig_different_users_sourceafsi/sourceafis_far_frr_data.txt};
\addlegendentry{Quadratic \FRR}
...
\end{axis}
\end{tikzpicture}
\caption{Empirical \FAR and \FRR as functions of the decision threshold, compared with the uniform and quadratic probabilistic distribution functions obtained from the KS fits.}
...
\label{fig:bio_only_sourceafis_reg_scoring}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.48\textwidth}
\centering
...
$0.82$ and $0.82$ at the \EER point, and 
$0.83$ and $0.84$ at their respective optimal operating points.
}
\label{fig:sourceafis_success}
\end{figure*}
}

We next evaluate mechanisms that use an atomic credential and a biometric credential. We simulate an atomic credential with arbitrary fault probabilities~$P^{1}_\text{safe} = 0.75$, $P^{1}_\text{leak} = 0.1$, $P^{1}_\text{loss} = 0.1$, and $P^{1}_\text{stolen} = 0.05$. Then, we calculate the success probability of the \emph{AND mechanism}~(Equation~\ref{eqn:andsuccess}), and that of the \emph{OR mechanism}~(Equation~\ref{eqn:orsuccess}). We illustrate the success probability functions of mechanisms, that use an atomic credential and a biometric credential as functions of~$t$ in Figure~\ref{fig:bio_atm_sourceafis_reg_scoring}. We show that the optimal threshold for each of the mechanisms differs from the~\EER point. Moreover, both mechanisms operating at their optimal thresholds achieve success probabilities that are~8-9\% higher than that of the atomic credential only mechanism, demonstrating the benefit of using the biometric credential in addition. 

However, the success probabilities of all evaluated mechanisms, when operated at their optimal thresholds, are not significantly greater than those of the corresponding mechanisms operating at the~\EER point. This can be explained by the fact that the~\EER of the biometric credential is already very low, indicating high separability between a user and an attacker and thus, leaving small room for improvement. In this scenario, we tested different users against each other. However, our main objective is to increase the mechanism's success probability in scenarios where an imposter attempts to spoof the user's templates, and authenticate to the user's account. Hence, we expect the imposter to use spoofing tools, producing samples whose score distribution is much more similar to the genuine distribution, which might lead to an increased~\EER point. 

\begingroup
\makeatletter
\setlength{\@dblfptop}{0.5in}
\makeatother
\sourceafisplotspanel
\clearpage
\endgroup

\newcommand{\livdetplotblock}{%
\begin{strip}
...
\centering
\begin{tikzpicture}
% Left axis: impostor counts
\begin{axis}[
    ybar,
    legend to name=livdethistogramlegend,
    bar width=2pt,
    width=0.86\linewidth,
    height=0.45\linewidth,
    xlabel={Normalized similarity score (\%)},
...
\addplot+[ybar,fill=red,draw=black]
table[x=score,y=impostor] 
{figs/fig_spoofed_users/livdet_sourceafis_histogram_best_of_probes_data.txt};
\end{axis}
% Right axis: genuine counts
\begin{axis}[
    ybar,
    legend to name=livdethistogramlegend,
    legend columns=2,
...
\addplot+[ybar,fill=blue,draw=blue,fill opacity=0.5]
table[x=score,y=genuine] 
{figs/fig_spoofed_users/livdet_sourceafis_histogram_best_of_probes_data.txt};
\addlegendentry{Genuine}
\addlegendimage{fill=red,draw=black,mark=none}
\addlegendentry{Impostor}
\end{axis}
\end{tikzpicture}
\smallskip
\pgfplotslegendfromname{livdethistogramlegend}
...
% Genuine: fitted Weibull PDF
\addplot[blue, thick, domain=0.864333:100, samples=300]
{(1.26817/29.1126) * ((x-0.864333)/29.1126)^0.26817 * exp(-((x-0.864333)/29.1126)^1.26817)};
\addlegendentry{Genuine: Weibull}

% Impostor: fitted logistic PDF
\addplot[red, thick, domain=0:100, samples=300]
{exp(-(x-4.23277)/2.67652) / (2.67652* (1+exp(-(x-4.23277)/2.67652))^2)};
\addlegendentry{Impostor: logistic}
\end{axis}
\end{tikzpicture}
\caption{Probability-density functions selected by the KS estimation.}
\label{fig:livdet_ks_estimation}
\end{subfigure}
...
% Empirical FAR: discrete histogram integral
\addplot[
    thick,
    red,
    const plot mark left,
...
% Empirical FRR: discrete histogram integral
\addplot[
    thick,
    blue,
    const plot mark left,
...
% Uniform FAR: continuous KS model
\addplot[
    thick,
    red,
    dashed,
...
table[x=T, y=FAR_uniform] {figs/fig_spoofed_users/livdet_sourceafis_far_frr_theoretical_data.txt};
\addlegendentry{Uniform \FAR}

% Uniform FRR: continuous KS model
\addplot[
    thick,
    blue,
    dashed,
...
table[x=T, y=FRR_uniform] {figs/fig_spoofed_users/livdet_sourceafis_far_frr_theoretical_data.txt};
\addlegendentry{Uniform \FRR}

% Quadratic FAR: continuous KS model
\addplot[
    thick,
    red,
    densely dotted,
...
table[x=T, y=FAR_quadratic] {figs/fig_spoofed_users/livdet_sourceafis_far_frr_theoretical_data.txt};
\addlegendentry{Quadratic \FAR}

% Quadratic FRR: continuous KS model
\addplot[
    thick,
    blue,
    densely dotted,
...
table[x=T, y=FRR_quadratic] {figs/fig_spoofed_users/livdet_sourceafis_far_frr_theoretical_data.txt};
\addlegendentry{Quadratic \FRR}

% Discrete empirical EER
\addplot[
    only marks,
    mark=*,
    mark size=1.6pt,
...
\caption{Empirical \FAR and \FRR as functions of the threshold, compared with the uniform and quadratic PDFs obtained from the KS fits. The empirical \EER point is marked and occurs at $\approx 22.1\%$.}
\label{fig:livdet_sourceafis_far_frr}
\end{subfigure}\hfill%
\begin{subfigure}[t]{0.48\textwidth}
\centering
\raisebox{7\baselineskip}[0pt][0pt]{%
...
\begin{tikzpicture}
\begin{axis}[livdetsuccess, xlabel={}]
...
\end{axis}
\end{tikzpicture}
\par\scriptsize Single biometric credential.\par
...
\end{axis}
\end{tikzpicture}
\par\scriptsize AND and OR mechanisms.\par
...
The AND mechanism achieves a maximal success probability of $0.834$, while the OR mechanism achieves $0.816$.
At the \EER operating point, the success probabilities are $0.713$ and $0.649$ for the AND and OR mechanisms, respectively.
}
\label{fig:livdet_sourceafis_success}
\end{minipage}%
}%
\end{subfigure}
\par\vspace{9\baselineskip}% Reserve space for the raised right-hand panel and its caption.
\caption{LivDet spoofing-attack evaluation using the best score over probe templates.}
\label{fig:livdet_evaluation_panel}
\end{minipage}
\end{strip}
}
...
\subsection*{\thesubsection.\quad Spoofing Attack Scenario}
\makeatletter
\immediate\write\@auxout{\string\newlabel{sec:7.2}{{\thesubsection}{\thepage}{Spoofing Attack Scenario}{\@currentHref}{}}}
\immediate\write\@auxout{\string\@writefile{toc}{\string\contentsline {subsection}{\string\numberline {\thesubsection}Spoofing Attack Scenario}{\thepage}{\@currentHref}\string\protected@file@percent}}
\makeatother
\noindent
...
We follow the same methodology as before and use the $\text{SourceAFIS}$ algorithm~\cite{sourceafis}. As reference images, we randomly select six Digital Persona scanner images of each user in the database, while the probe set of each user comprises the remaining real and fake images. The geniune comparisons are all comparisons between the reference and real images in the probe set.
For each probe image, we define the similarity score as the maximum score among 6 reference images. 
We simulate a spoofing attack by using fake fingerprint images of each user produced by~\textit{Gelatine}, \textit{Woodglue}, \textit{Latex} and~\textit{Ecoflex} and match them against the reference images to obtain the imposter score distribution. We build the resulting histogram of a user and an imposter (Figure~\ref{fig:livdet_sourceafis_histogram}) and then calculate the~\FAR-\FRR error rates to compute the~\EER point (Figure~\ref{fig:livdet_sourceafis_far_frr}). As we expect, the~\EER increased significantly to 22.1\% indicating the user and imposter distributions are less separable and more overlapping in spoofing attacks. To understand the implications of a spoofing attack, we calculate the success probability of a mechanism uses a biometric credential and a mechanism uses an atomic credential and a biometric credential, as shown in Figure~\ref{fig:livdet_sourceafis_success}. We find that the optimal operating point reducing the failure probability between 18\% to 70\% compared to the~\EER point when the mechanism uses both an atomic and a biometric credential. Moreover, we find that a mechanism uses an atomic credential and a biometric credential at the~\EER threshold, might achieve lower success probability than that of a mechanism uses an atomic credential only, or a biometric credential only. These results demonstrating that non-optimizing the threshold of the biometric credential, when using multiple credentials in a mechanism, might drawback the mechanism's success probability. These results are summarized in Table~\ref   {tab:psafe_sweep_improvement}.
...
\captionsetup{type=table}
    \centering
    \caption{Comparison between the success probabilities obtained at the EER threshold and the maximal success probabilities. Bold EER values indicate the worse composition, whereas bold maximal values indicate the best composition. The bold safe column indicating the success probability of a mechanism uses an atomic credential only. The improvement column shows the percentage of improvement between the failure probability at the optimal mechanism success and the failure probability at the maximal mechanism success at EER threshold.}
    ...
    \begin{tabular}{cccccccccc}
        ...
        \midrule
        \textbf{0.60} & 0.05 & 0.30 & 0.05 & 0.64 & \textbf{0.49} & \textbf{0.71} & 0.62 & 18.03\% \\
        \textbf{0.72} & 0.05 & 0.18 & 0.05 & 0.67 & \textbf{0.59} & \textbf{0.76} & 0.74 & 30.05\% \\
        \textbf{0.71} & 0.19 & 0.05 & 0.05 & \textbf{0.58} & 0.66 & 0.71 & \textbf{0.79} & 38.60\% \\
        \textbf{0.82} & 0.08 & 0.05 & 0.05 & \textbf{0.66} & 0.68 & 0.82 & \textbf{0.85} & 53.23\% \\
        \textbf{0.90} & 0.05 & 0.00 & 0.05 & \textbf{0.70} & 0.73 & 0.89 & \textbf{0.92} & 70.59\% \\
        \bottomrule
    \end{tabular}
\end{minipage}
\end{strip}
...
\subsection{Spoofing Attack With Multiple Attempts}\label{sec:7.3}
\noindent
We simulate a spoofing attack scenario in which a player has multiple attempts to authenticate. We use the same genuine and impostor distributions calculated in the previous section~$(\S\ref{sec:7.2})$ using the LivDet2015 database. 

We follow the theoretical analysis in~$(\S\ref{sec:4.2})$, and calculate the success probability of a mechanism that uses a biometric credential only~(Equation~\ref{eqn:safesuccess}). Then, we simulate several authentication attempts of a player~$P$. For each number of attempts~$k$, we calculate the probability of a successful authentication as the probability that the user is accepted at least once, while the attacker is rejected over all~$k$ attempts. As the success probability becomes dependant on the number of attempts~$k$, we compare the mechanism success probability under three baselines of threshold selections: (1)~the~\EER threshold reused for all~$k$ attempts, (2)~the one attempt optimal threshold reused for all~$k$ attempts, and (3)~threshold optimization over all~$k$ attempts, as shown in Figure~\ref{fig:Success_probability_by_k}.

We find that the success probability of a mechanism that uses a biometric credential only, increases with the number of attempts~$k$, when optimizing the threshold overl all~$k$ attempts. Futhermore, we can see that the success probability when reusing the~\EER threshold for all~$k$ attempts, is reducing with the number of attempts~$k$, indicating that the attacker is more likely to succeed in spoofing the biometric credential as the number of attempts increases.

We next consider a different policy where the user is allowed~$k_u$ authentication attempts and the attacker is allowed~$k_a$ spoofing attempts. We use the~\FAR and~\FRR estimated from the LivDet2015 database in previous section~$(\S\ref{sec:7.2})$ and fix the number of attempts for the attacker to~$k_a=10$ and the number of attempts for the user to~$k_u=3$. These selections simulate a real-world scenario that represents the typical number of attempts for a user authentication versus the maximal allowed attempts forced by the mechanism~\cite{typical_aut_attempts}. We examine how the threshold selection affects the success probability of a mechanism that uses a biometric credential only. Under this policy, we present the success probability as function of the number of attempts in three baselines: (1)~the~\EER threshold reused for all~$k$ attempts, (2)~the one attempt optimal threshold reused for all~$k$ attempts, (3)~threshold optimization over all~$k$ attempts, as shown in Figure~\ref{fig:fixed_user_attacker_success_by_k}. We find that for the first three attempts, the optimal threshold doesn't change and is optimized repectively for~$k_u$. That means different user attempts lead to different optimal thresholds over~$k_u$. However, as the attempt index increases above~$k_u$, the threshold increases to reduce the probability of a successful spoofing attack. We also find that the success probability of a mechanism that uses a biometric credential only, is significantly higher when using the optimal threshold selection over~$k_u$ attempts, compared to reusing the~\EER threshold for all~$k_u$ attempts. 
...
% Maximum cumulative success probability
\addplot[green!60!black, solid, mark=*,mark size=1.6pt]
table[x=k, y=P_success_max] {figs/fig_spoofed_users/livdet_fixed_prefix_far_frr_success_by_k.txt};
\addlegendentry{$P_{\mathrm{success}}^{\max}$}

% EER threshold reused for all k attempts
\addplot[blue, dashed, mark=square*,mark size=1.6pt]
table[x=k, y=P_success_EER] {figs/fig_spoofed_users/livdet_fixed_prefix_far_frr_success_by_k.txt};
\addlegendentry{$P_{\mathrm{success}}(t_{\mathrm{EER}})$}

% Single-attempt optimal threshold reused for all k attempts
\addplot[red, densely dotted, mark=triangle*,mark size=1.6pt]
table[x=k, y=P_success_single_attempt_optimum] {figs/fig_spoofed_users/livdet_fixed_prefix_far_frr_success_by_k.txt};
\addlegendentry{$P_{\mathrm{success}}(t_{\mathrm{opt}}^{(1)})$}

% Product of the false-rejection rates
\addplot[orange, solid, mark=diamond*,mark size=1.6pt]
table[x=k, y=product_FRR] {figs/fig_spoofed_users/livdet_fixed_prefix_far_frr_success_by_k.txt};
\addlegendentry{$\prod_{j=1}^{k}\mathrm{FRR}_j$}

% Product of the attacker-rejection probabilities
\addplot[purple, solid, mark=pentagon*,mark size=1.6pt]
table[x=k, y=product_1_minus_FAR] {figs/fig_spoofed_users/livdet_fixed_prefix_far_frr_success_by_k.txt};
\addlegendentry{$\prod_{j=1}^{k}(1-\mathrm{FAR}_j)$}

\end{axis}
\end{tikzpicture}
\caption{The maximal cumulative success probability as a function of the number of authentication attempts~$k$, compared with reusing the \EER threshold and the single-attempt optimal threshold for all attempts. The products of the false-rejection rates and attacker-rejection probabilities of the optimized policy are also shown.}
\label{fig:Success_probability_by_k}
\end{figure}
...
\addplot[green!60!black, solid, mark=*]
table[x=k, y=P_success_optimized] {figs/fig_spoofed_users/livdet_fixed_user_attacker_success_by_k.txt};
\addlegendentry{Optimized thresholds}

\addplot[blue, dashed, mark=square*]
table[x=k, y=P_success_EER] {figs/fig_spoofed_users/livdet_fixed_user_attacker_success_by_k.txt};
\addlegendentry{EER threshold}

\addplot[red, densely dotted, mark=triangle*]
table[x=k, y=P_success_single_attempt_optimum] {figs/fig_spoofed_users/livdet_fixed_user_attacker_success_by_k.txt};
\addlegendentry{Single-attempt optimum}

\end{axis}
\end{tikzpicture}
\caption{The cumulative success probability through attempt~$k$ for the fixed attempt limits $k_u=3$ and $k_a=10$. The optimized policy uses independently optimized per-attempt thresholds. The two baselines reuse either the EER threshold or the single-attempt-optimal threshold at every active attempt. After $k=3$, the legitimate user's success term remains fixed while additional attacker attempts continue to affect the attacker-rejection term.}
\label{fig:fixed_user_attacker_success_by_k}
\end{figure}

\section{Conclusion}\label{sec:8}
\noindent
We find that the optimal threshold of a biometric credential depends on the other credentials in the mechanism and differs from the heuristic threshold commonly used today. We characterize a condition on the biometric credential and prove that whenever this condition holds, a mechanism designer always benefits from adding a biometric credential over using atomic credentials alone. We quantify this theoretical result by simulating different scenarios using public biometric datasets, including spoofing attacks. Our results show that optimizing the threshold significantly reduces the failure probability under spoofing attacks by~18-70\%.

% We introduce a formal model of authentication mechanisms with both biometric and atomic credentials. We find that the optimal threshold of a biometric credential depends on the other credentials in the mechanism and differs from the heuristic threshold commonly used today. We then prove that using an asymptotically separable biometric credential and~$k$ atomic credentials in a mechanism achieves greater success probability than a mechanism that uses the atomic credentials only. This result indicates that a mechanism designer will always benefit from adding an asymptotically separable biometric credential into a mechanism that uses atomic credentials only.

%We quantitatively simulate different scenarios on public biometric datasets, 
%including spoofing attacks. Our results show that optimizing the threshold significantly reduces the failure probability when a spoofing attack occurs by~30-70\%. 

However, current biometric sensors prevent mechanism designers and integrators from selecting the threshold, achieving a suboptimal mechanism success probability. This result paves the way for developing new authentication protocols based on threshold selection. We further propose that biometric sensor vendors should change their interface and let the integrators set the thresholds of the biometric credentials. Harnessing our implication may enhance the security of digital and physical assets for millions of users around the world.
