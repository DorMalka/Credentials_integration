...
\begin{definition}(Availability Vector)
    ...
    \end{equation*}
\end{definition}
The outcome of the presentation function determines whether~$\sigma_i^P=1$ or $\sigma_i^P=0$. Denote the set of credentials available to player~$P$ by $\sigma^P$.
...
\begin{definition} (Biometric Credential)~For a biometric credential the presentation function~$\textit{present}_P(i)$ is defined by the user score distribution~$f_U(s)$, attacker score distribution~$f_A(s)$ and a match threshold~$t$. For a player~${P\in\{U,A\}}$, the allocation function~$\textit{allocate}_P(i)$ draws a random value for the score~$s$ from the distribution~$f_P(s)$, and outputs~1 if~$s\ge t$ and~0 otherwise.
    \begin{equation*}
    s = \textit{allocate}_P(i) \sim f_P(s)
    \end{equation*}
    \begin{equation*}
...
\begin{itemize}
     \item~$\sigma^P$ the set of credentials available to player~$P$ in scenario~$\sigma$ (perhaps an empty set, represented by~$\varnothing$).
\end{itemize}
...
\begin{definition} (Mechanism Success) A mechanism~$M$ is \emph{successful} in a scenario~$\sigma$ if for a player~$P$ and the set of available credentials~$\sigma^P$, the authentication mechanism decision~$\mathrm{decide}_M(\sigma^P)$ equals~1 for the user only over an execution~$E$, that is
\begin{equation*}
