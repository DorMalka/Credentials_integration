...
Biometric credentials are increasingly adopted in modern user devices. They rely on unique physical properties that make them difficult to forge~\cite{biometric_importance}. These properties have made them widely adopted, with~81\% of smartphones and~70\% of financial banking services relying on face or fingerprint verification~\cite{most_smartphones_biometric,financial_biometric} for access control. However, they were not always applicable~\cite{Anil1997} and therefore, credentials were typically \emph{atomic}, a credential was either presented correctly or not, evaluated according to a perfect match criterion.
...
Biometric credentials are evaluated by a sensor. The sensor obtains a~\emph{sample}, calculates a \emph{similarity score} between that sample and a~\emph{reference} stored in advance, and outputs~\emph{accept} if the score is above a set~\emph{match threshold} or \emph{reject} otherwise. In practice, however, sensors typically expose only this binary outcome and do not allow the authentication mechanism to tune the match threshold. A \emph{genuine distribution}~$f_U(s)$ describes the score distribution of samples that belong to the legitimate user, whereas the \emph{impostor distribution}~$f_A(s)$ describes the score distribution of samples that belong to the attacker, as shown in Figure~\ref{fig:intro-fig}. These distributions establish two error metrics known as~\FAR and~\FRR. The~\FAR is the probability that the sensor outputs \emph{accept} for an attacker sample. We calculate it by integrating the impostor distribution from the match threshold to the maximum score, which corresponds to the red area under the curve in Figure~\ref{fig:intro-fig}. Similarly, the~\FRR is the probability that the sensor outputs \emph{reject} for a user sample. We calculate it by integrating the genuine distribution from the minimum score to the match threshold, which corresponds to the blue area under the curve in Figure~\ref{fig:intro-fig}.
...
We start by optimizing a single biometric credential mechanism~$(\S\ref{sec:4})$. We derive closed-form expressions of the mechanism's success probability, which in a single credential mechanism, reduces to the probability that the credential is \emph{safe}, i.e., available only to the user. We identify the optimal threshold of the biometric credential and show that, in general, the optimal threshold differs from the commonly used~\EER threshold. This difference becomes more significant when the attacker is more likely to produce higher matching scores~$s$ and when the user and attacker distributions are more asymmetric.
...
We then turn to analyze mechanisms that integrate a biometric and an atomic credential. We derive closed-form expressions for the mechanism's success probability. We symbolically calculate the optimal and~\EER thresholds and find that the optimal threshold depends on the values of the atomic credential. These expressions reveal that the mechanism's success can primarily depend on either an atomic credential or a biometric credential, depending on their properties. For example, in a mechanism that requires all credentials known as an~\emph{AND mechanism} the mechanism grants access to a user if and only if all credentials are available to the user and at least one credential is unavailable to the attacker. The derived relation for this mechanism shows that when the atomic credential has arbitrarily low fault probabilities, the mechanism’s success probability depends primarily on the atomic credential. In this case, biometric samples do not significantly improve the overall mechanism success probability. Conversely, for other probability values of an atomic credential, the biometric credential might improve the mechanism success probability more significantly.

Motivated by this observation, we characterize the conditions under which the integration of a biometric credential with an atomic credential will always enhance the mechanism's success probability~$(\S\ref{sec:6})$. First, we show that when an atomic credential has zero \emph{loss} or \emph{leak} state probabilities, it is not worthwhile to integrate, as the atomic credential mechanism dominates the mechanism success probability. Then, we introduce a necessary and sufficient condition for an \emph{asymptotically separable} biometric credential: for every score above or below a match threshold, the error rates converge to zero. We prove that every \emph{asymptotically separable} biometric credential integrated with an atomic credential that has non-zero probabilities for the states of \emph{safe} and either \emph{loss} or \emph{leak}, improves the mechanism success probability. However, biometric credentials often exhibit a more common and weaker behavior~\cite{Slob2005,anil2004}. Thus, we introduce \emph{asymptotic dominance}: the genuine distribution dominates the impostor distribution by a factor~$c$ for every score above a specific threshold, or the impostor distribution dominates the genuine distribution by a factor~$c$ for every score below a specific threshold. We show that this condition is sufficient for a biometric credential to improve the mechanism's success probability when integrated with an atomic credential.
...
\subsection{A Sufficient Condition For Integrating Biometric Credentials}\label{sec:6.3}
