...
%%%
%%%
To analyze the case of a mechanism $M$ comprised of two biometric credentials, we conduct a numerical study over several probability distribution functions. For each distribution pair, we derive the optimal operating point for both \emph{AND} and \emph{OR} mechanisms by maximizing the respective success probabilities using a LBFGS optimization algorithm. We additionally compare the optimal operating point to the~\EER point, thereby characterizing the conditions under which the \emph{AND} or \emph{OR} mechanisms yields superior performance. 
...
\left[
{P_{\text{safe1}}} + {P_{\text{loss1}}}(1 - \dfrac{t - u_1}{u_2 - u_1})
\right].
\]
\[
P_{\text{success}}(t) =
...
\dfrac{t - a_1}{a_2 - a_1}\,({P_{\text{safe1}}} + {P_{\text{loss1}}}),
& a_1 < t \le u_1 \\[10pt]
...
\end{cases}
\]
We illustrate the mechanism's success probability for both \emph{OR} mechanism in figure~\ref{fig:OR_mechanism_success} and \emph{AND} mechanism in Figure~\ref{fig:AND_mechanism_success}. The figure marks the optimal operating point and the~\EER point on each curve demonstrating significant improvement from~\EER using optimal threshold. 
...
\[t_{\text{opt}} = \frac{a_1\cdot{P_{\text{loss1}}} + u_2\cdot({P_{\text{safe1}}} + {P_{\text{loss1}}}) - {P_{\text{safe1}}}\cdot u_1}{2{P_{\text{loss1}}}}\]
And the maximum probability of success at this point is:
\[P_{\text{success}}(t_{\text{opt}}) = \frac{\big(a_1 {P_{\text{loss1}}} - u_2 ({P_{\text{safe1}}} + {P_{\text{loss1}}}) + {P_{\text{safe1}}} u_1\big)^2}
{4 {P_{\text{loss1}}} (a_2 - a_1)(u_2 - u_1)}\]
