...
\noindent
We will now characterize the conditions under which a mechanism that uses both a biometric credential and an atomic credential mechanism achieves greater success than a mechanism that uses only the atomic credential. We show that any mechanism using a single atomic credential with~${P^1_\text{loss} = P^1_\text{leak} = 0}$ and a biometric credential does not achieve greater success than a mechanism using this atomic credential~$(\S\ref{sec:6.1})$. Next, we present a necessary and sufficient condition for such improvement~$(\S\ref{sec:6.2})$ and derive a condition on the distributions of the user and attacker of the biometric credential that provides a sufficient condition~$(\S\ref{sec:6.3})$.
\subsection{No Improvement In Mechanism Success}\label{sec:6.1}
\noindent
We first present a condition under which adding a biometric credential into a mechanism~$M$ that uses only an atomic credential, cannot produce a new mechanism~$\bar{M}$ whose success probability is greater than that of the atomic credential alone.
\begin{proposition}
...
\noindent 
We define the condition of \emph{asymptotic separability} for a biometric credential. Intuitively, asymptotic separability means that there exists a threshold such that one error rate becomes negligible while the corresponding correct rate is not, making the user and attacker distributions effectively separated.
\begin{definition}
...
\end{equation*}
\end{definition}
We now consider an atomic credential with non-zero probabilities~$ P^1_\text{safe}$ and at least one of~$P^1_\text{leak}$ $\text{or } P^1_\text{loss}$. We show that asymptotic separability of a biometric credential is a necessary and sufficient condition for producing a new mechanism~$\bar{M}$ with greater success probability than that of~$M$. The resulting mechanism~$\bar{M}$ uses both an atomic credential and a biometric credential in either an \emph{AND} or \emph{OR} composition.
...
\end{proof}
We showed that \emph{asymptotic separability} is a sufficient condition. We now show it is a necessary condition for producing a new mechanism~$\bar{M}$ with greater than that of~$M$.
\begin{proposition}
    Let~$c_1$ be an atomic credential with~$P^1_\text{safe} > 0$ and at least one of~$P^1_\text{leak},P^1_\text{loss} > 0$, $c_2$ be a biometric credential with user distribution~$f_U(t)$ and attacker distribution~$f_A(t)$ and~$\bar{M}$ be a mechanism which is an \emph{AND} or \emph{OR} composition using both~$c_1$ and~$c_2$. Suppose there exists~$t$ such that
    \begin{equation*}
...
\end{proposition} 
\begin{proof}
    Assume by contradiction that the biometric credential is not asymptotically separable. Then, there exists~$\varepsilon > 0$ such that for all~$t$ both~$\frac{\FAR(t)}{1-\FRR(t)} \ge \varepsilon$ and~$\frac{\FRR(t)}{1-\FAR(t)} \ge \varepsilon$. Since it is given that~$\bar{M}$ using both credentials~$c_1$ and~$c_2$ and achieves a strictly greater success probability than an atomic credential mechanism~$M$, then there exists a threshold~$t$ such that either:~$P^{\text{AND}}_{\text{success}}(t) > P^1_{\text{safe}} \textit{ or }$ $P^{\text{OR}}_{\text{success}}(t) > P^1_{\text{safe}}.$ Thus, for every value of~$P^{1}_{\text{safe}}, P^{1}_{\text{leak}}, P^{1}_{\text{loss}}$ in the range~$(0,1)$ necessarily either the inequality of \textsc{AND} mechanism holds~(Equation~\ref{eqn:necessary term_1}) or the inequality of \textsc{OR} mechanism holds~(Equation~\ref{eqn:necessary term_2}). In particular, we can choose an atomic credential with state probabilities~$P^{1}_{\text{safe}}, P^{1}_{\text{leak}}, P^{1}_{\text{loss}}$ such that
    \begin{equation*}
        \max\left\{\frac{P^{1}_{\text{leak}}}{P^{1}_{\text{safe}}}, \frac{P^{1}_{\text{loss}}}{P^{1}_{\text{safe}}}\right\} \le \varepsilon.
    \end{equation*}
    ...
    \begin{equation*}
        \begin{aligned}
            &\frac{\FRR(t)}{1-\FAR(t)} \ge \varepsilon \ge \frac{P^{1}_{\text{leak}}}{P^{1}_{\text{safe}}} \textit{, and } \\
            &\frac{\FAR(t)}{1-\FRR(t)} \ge \varepsilon \ge \frac{P^{1}_{\text{loss}}}{P^{1}_{\text{safe}}}.
        \end{aligned}
    \end{equation*} 
...
\end{proof}
\noindent
We showed that~$\bar{M}$ achieves greater success probability than that of~$M$ if and only if the biometric credential is \emph{asymptotically separable} and the atomic credential has~$P^1_\text{safe} > 0$ and at least one of~$P^1_\text{leak},P^1_\text{loss} > 0$.
