...
\noindent
We now empirically evaluate different mechanisms that use an atomic credential and a biometric credential. To simulate a biometric credential, we use the public real-world~\textit{FVC2004} dataset of fingerprints~\cite{fvc2004}. This dataset contains multiple fingerprints scans of each user, taken using different types of biometric sensors. We divide each user's scans in the database into a probe set~- simulating the reference scans stored in advance, and a candidate set~- simulating the scans obtained by the sensor. To score the fingerprint images, we use a standard fingerprint verification algorithm~\textit{SourceAFIS}~\cite{sourceafis}, which is based on minutiae triplets, and evaluate all users who appear in~$\textit{FVC2004}$ \textit{Database1}. For each user, we randomly select three fingerprint templates as the probe set, while the remaining templates in the database form the candidate set. We compute a similarity score for each candidate scan by running the algorithm against each of the three probes, and choose the maximum score as the candidate final score. For example, if the matching scores between a candidate scan and the three probes are~$[0.1,0.6,0.9]$, then the resulting score for this candidate is~$0.9$. The geniune comparisons are defined as all comparisons between the user probes and the users' remaining scans appear in~\textit{Database1}. In contrast, the imposter comparisons are defined as all comparisons between a user's probes and the scans of the other users in~\textit{Database1}.

We follow the process used to evaluate the accuracy of the biometric sensor, as presented in the experimental results of Jain et al.~\cite{Anil1997}, Cappelli et al.~\cite{Cappelli2006} and Slobodan et al.~\cite{Slob2005}. We produce histograms of the geniune and imposter comparisons, as shown in Figure~\ref{fig:sourceafis_histogram}. Then, we convert the histograms into smoothed probability density functions and calculate the~\FAR and~\FRR as functions of the threshold~$t$, as shown in Figure~\ref{fig:sourceafis_far_frr}.

The algorithm we use acheives an~\EER of approximately~2\%, similar to the best results reported in~\textit{FVC2004} competition~\cite{fvc2004}, and to those achieved by sensors used in industry and academia when evaluating different users~\cite{Maio2002,Cappelli2006}. This allows us to objectively compare our results with those of state-of-the-art fingerprint verification algorithms. We calculate the biometric credential fault probabilities~(Equation~\ref{eqn:safe_prob}) and the mechanism success probability uses a biometric credential only~(Equation~\ref{eqn:safesuccess}). We find that the optimal threshold differs from the~\EER point and achieves a higher success probability, as shown in Figure~\ref{fig:bio_only_sourceafis_reg_scoring}. 
...
Finally, we evaluate mechanisms that use an atomic credential and a biometric credential. We simulate an atomic credential with arbitrary fault probabilities~$P^{1}_\text{safe} = 0.75$ and~$P^{1}_\text{leak} = 0.1$, $P^{1}_\text{loss} = 0.1$ and $P^{1}_\text{stolen} = 0.05$. Then, we calculate the success probability of an \emph{AND mechanism}, as described by Equation~\ref{eqn:andsuccess}, and that of an \emph{OR mechanism}, as described by Equation~\ref{eqn:orsuccess}. We illustrate the success probability functions of mechanisms that use an atomic credential and a biometric credential in Figure~\ref{fig:bio_atm_sourceafis_reg_scoring}. We show that the optimal threshold for each of the mechanisms differs from the~\EER point. Moreover, the mechanisms operating at their respective optimal threshold, achieve a higher success probability of approximately~2\% compared to operating at the~\EER point. In addition, both mechanisms operating at their optimal threshold, achieve a higher success probability than the atomic credential only mechanism by~8-9\%, as observed in Figure~\ref{fig:bio_atm_sourceafis_reg_scoring}. 

However, the success probability of all mechanisms evaluated, when operated at their optimal thresholds, is not significantly greater than that of the corresponding mechanisms operating at the~\EER point. This can be explained by the fact that the~\EER of the biometric credential is already very low, indicating high separability between a user and an attacker and thus, leaving small room for improvement. In this scenario, we tested different users against each other. However, our main objective in this paper is to increase the mechanism's success probability in scenarios where an imposter attempts to spoof the user's templates, and authenticate to the user's account. Hence, we expect the imposter to use spoofing tools, producing samples whose score distribution is much more similar to the genuine distribution, which might lead to an increased~\EER point. 
...
We follow the same methodology as before and use the $\text{SourceAFIS}$ algorithm~\cite{sourceafis}. As probe templates, we select six Digital Persona scanner images of user 002\_0. Each probe was matched against 14 additional images of the same user to generate genuine scores. 
For each candidate scan, we define the similarity score as the maximum score among 6 probe templates. 
\begin{table*}[t]
    \centering
