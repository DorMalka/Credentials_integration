...
We will now characterize the conditions under which integrating a biometric credential into a single atomic credential mechanism, enhances its success. We show that for any mechanism type whose atomic credential characterized by non-zero probabilities~$ P^1_\text{safe} \text{ and } P^1_\text{stolen}$ only, integrating a biometric credential does not improve mechanism's success~$(\S\ref{sec:6.1})$. Next, we present a necessary and sufficient condition for such improvement~$(\S\ref{sec:6.2})$ and derive a more commong criterion that provides a sufficient condition~$(\S\ref{sec:6.3})$.
\subsection{No Improvement In Mechanisms Success}\label{sec:6.1}
\begin{proposition}
    For any mechanism~$M$ comprises an atomic credential with non-zero probabilities~$ P^1_\text{safe}$ and~$ P^1_\text{stolen}$ only, integrating the mechanism with a biometric credential does not enhance its mechanism's success.
\end{proposition}
\begin{proof}
    Integrating a biometric credential into a mechanism~$M$ comprises an atomic credential with non-zero probabilities~$ P^1_\text{safe}$ and~$ P^1_\text{stolen}$ only, leads to mechanism success probability described by Equation~\ref{eqn:andsuccess} or Equation~\ref{eqn:orsuccess}. Since the state probabilities of an atomic credential are~$ P^1_\text{leak} =  P^1_\text{loss} = 0$, the success probability in both mechanism cases reduces to:

    \[P_{\text{success}}(t) = P^1_\text{safe} \cdot P^2_{\text{safe}}(t).\]
    Where~$P^2_{\text{safe}}(t)$ is the safe probability of the biometric credential as a function of the operating threshold $t$.
    However, $P^2_{\text{safe}}(t) \le 1$ for every value of $t$. Then $P_{\text{success}}(t) \le P^1_\text{safe}$ and thus, integrating a biometric credential does not enhance the success of the mechanism.
\end{proof}
\subsection{Sufficient And Necessary Condition for Integrating Biometric Credentials}\label{sec:6.2}
First, we define the condition of \emph{asymptotic separability} of a biometric credential. Then, we will show that its a necessary and sufficient condition for enhancing the mechanism's success when integrating a biometric credential.
\begin{definition}
A biometric credential with user distribution function $f_U(s)$
and attacker distribution function $f_A(s)$ is said to be \emph{asymptotically separable} if
\[
\inf_{t}
\min\!\left\{
\frac{\FAR(t)}{1-\FRR(t)},\,
...
= 0,
\]
where $\FAR(t) = \int_{t}^{\infty} f_A(s) ds$ and $\FRR(t) = \int_{-\infty}^{t} f_U(s) ds$.
\end{definition}

\begin{proposition}
    For any mechanism $M$ comprises an atomic credential with non-zero probabilities~$P^1_\text{safe}$ $\text{and}$ $P^1_\text{leak}$ $\text{or}$ $P^1_\text{loss}$, integrating the mechanism with a biometric credential whose user and attacker distributions are asymptotically separable, enhances the success of the mechanism.
\end{proposition}

\begin{proof}
    The success probability of a mechanism comprises an atomic credential only is given by Equation~\ref{eqn:safesuccess}. A biometric credential is characterized by four state probabilities which are function of~$t$: $P^{2}_{\text{safe}}(t)$, $P^{2}_{\text{loss}}(t)$, $P^{2}_{\text{leak}}(t)$, $P^{2}_{\text{theft}}(t)$. We can describe each one of these probabilities as a function of~\FAR and ~\FRR as:
    \contprobabilities
    The success probability of the integrated mechanism comprises two credentials is described by Equation~\ref{eqn:andsuccess} and Equation~\ref{eqn:orsuccess}. Thus, by placing the biometric credential in each configuration we can express the success probability of the integrated mechanism as a function of the operating threshold $t$:
    \begin{equation}
        \label{eqn:andineq}
    \ANDintegration
    \end{equation}
    ...
    \ORintegration
    \end{equation}
    To prove the integration of a biometric credential improved mechanism success, we need to prove that there exists an operating threshold $t$ such that either 
    $P^{\text{AND}}_{\text{success}}(t) > P^1_{\text{safe}}$ or $P^{\text{OR}}_{\text{success}}(t) > P^1_{\text{safe}}$.
    By algebraic manipulation of Equation~\ref{eqn:andineq} we will demand that~$P^{\text{AND}}_{\text{success}}(t) > P^1_{\text{safe}}$ if and only if:
    \[
    1-\FRR(t) > \frac{P^{1}_{\text{safe}}}{P^{1}_{\text{safe}}+P^{1}_{\text{leak}}\cdot(1-\FAR(t))} \iff
    \]
    \[
...
\begin{proposition}
Suppose there~$\exists t$ such that the integrated mechanism~$M$ comprises a biometric credential and an atomic credential with non zero probabilities~$P_\text{safe}$ $\text{and}$ $P_\text{leak}$ $\text{or}$ $P_\text{loss}$ achieves a strictly higher success probability than an atomic credential mechanism, i.e.,\[P^{\text{integrated}}_{\text{success}}(t) > P^{\text{atomic}}_{\text{success}}.\] Then necessarily, the biometric credential is asymptotically separable.
\end{proposition} 

\begin{proof}
    We prove by contradiction. Assume for the sake of contradiction, that the biometric credential is not asymptotically separable. Then $\exists \varepsilon > 0$ such that $\forall t$ both $\frac{\FAR(t)}{1-\FRR(t)} \ge \varepsilon$ and $\frac{\FRR(t)}{1-\FAR(t)} \ge \varepsilon$. Since it's given that the integrated mechanism $M$ achieves a strictly higher success probability than an atomic credential mechanism, then there exists a threshold $t$ such that either:~$P^{\text{AND}}_{\text{success}}(t) > P^1_{\text{safe}} \textit{ or }$ $P^{\text{OR}}_{\text{success}}(t) > P^1_{\text{safe}}.$ Thus, either Equation~\ref{eqn:FRRineq} or Equation~\ref{eqn:FARineq} holds for every value of~$P^{1}_{\text{safe}}, P^{1}_{\text{leak}}, P^{1}_{\text{loss}}$ in the range of~$(0,1)$. In particular, we can choose~$P^{1}_{\text{safe}}, P^{1}_{\text{leak}}, P^{1}_{\text{loss}}$ such that either
    \[\frac{P^{1}_{\text{leak}}}{P^{1}_{\text{safe}}+P^{1}_{\text{leak}}} < \varepsilon \textit{ or } \] 
    \[\frac{P^{1}_{\text{loss}}}{P^{1}_{\text{safe}}+P^{1}_{\text{loss}}} < \varepsilon.\] 
    Since the equation holds~$\forall \varepsilon > 0$, but both~$\frac{\FAR(t)}{1-\FRR(t)} \ge \varepsilon$ and $\frac{\FRR(t)}{1-\FAR(t)} \ge \varepsilon$ derive by the sake of contradiction assumption, we can deduce that
    \[\frac{\FRR(t)}{1-\FAR(t)} \ge \frac{P^{1}_{\text{leak}}}{P^{1}_{\text{safe}}+P^{1}_{\text{leak}}} \textit{ and } \]
    \[\frac{\FAR(t)}{1-\FRR(t)} \ge \frac{P^{1}_{\text{loss}}}{P^{1}_{\text{safe}}+P^{1}_{\text{loss}}}.\] 
    Which contradicts both Equation~\ref{eqn:FRRineq} and Equation~\ref{eqn:FARineq} under the strictly higher success probability assumption. Therefore, the biometric credential must be asymptotically separable.

\end{proof}
We show that \emph{asymptotic separability} of the user and attacker distributions is a sufficient and necessary condition for enhancing the success of a mechanism comprises an atomic credential with non zero probabilities $P_\text{safe}, P_\text{leak} \text{ or } P_\text{loss}$ by integrating it with a biometric credential.

\subsection{A Sufficient Condition For biometric Credentials}\label{sec:6.3}
We define a \emph{asymptotic dominancy}, which is a more common condition of a biometric credential, and prove this is a sufficient condition for enhancing the success of an atomic credential mechanism.
\begin{definition}
\label{def:User and Attacker distribution functions}
A biometric credential with user a distribution fucntion~$f_U(s)$ and an attacker distribution function~$f_A(s)$
...
For any mechanism $M$ comprises an atomic credential and mechanism success probability $P^{\text{atomic}}_{\text{success}}$, integrating the mechanism with an asymptotically dominated biometric credential improves its achievable success. That is
\[
...
The success probability of a mechanism comprises an atomic credential only is given by Equation~\ref{eqn:safesuccess}. A biometric credential is characterized by four state probabilities which are function of~$t$: $P^{2}_{\text{safe}}(t)$, $P^{2}_{\text{loss}}(t)$, $P^{2}_{\text{leak}}(t)$, $P^{2}_{\text{theft}}(t)$. We can describe each one of these probabilities as a function of~\FAR and ~\FRR as:
\contprobabilities
The success probability of the integrated mechanism comprises two credentials is described by Equation~\ref{eqn:andsuccess} and Equation~\ref{eqn:orsuccess}. Thus, we can express the success probability of the integrated mechanism as a function of the operating threshold~$t$ as shown in Equation~\ref{eqn:andineq} and Equation~\ref{eqn:orineq}. We previously observed that a sufficient condition for improvement of mechanism's success is described by Equation~\ref{eqn:FRRineq} and Equation~\ref{eqn:FARineq}. Thus, we need to show that there exists an operating threshold $t$ such that either:
...
\]
We showed that~$\forall c>0$  either~$\frac{\FAR(t)}{1-\FRR(t)} < c$ or $\frac{\FRR(t)}{1-\FAR(t)} < c$. Therefore, integrating an asymptotically dominated biometric credential with an atomic credential improves mechanism success probability.
\end{proof}

This proof is not trivial, as the integration of another atomic credential to an atomic credential mechanism, does not always enhance its success. Moreover, we have shown when integrating biometric credential into an atomic credential mechanism without adjusting the match threshold~$t$ might reduce the mechanism's success probability~$(\S\ref{sec:5.2})$. Thus, Proposition~\ref{thm:biometric-improves-security} shows that by using an \emph{asymptotically dominated} biometric credential, we can always improve the success of all mechanism's composed of an atomic credential.
This condition supports practical scenarios, as we expect that for very low values of score~$s$ the attacker distribution dominates the user, vice versa for very high values of~$s$. We supports this claim with real public biometric data in Section~\ref{sec:7}.
