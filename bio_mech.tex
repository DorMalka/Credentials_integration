...
Biometric credentials are prevalent in modern authentication.  However, in the early days of the Internet, biometric credentials were not applicable and therefore, credentials were typically \emph{atomic}, a credential was either presented correctly or not according to a perfect match criterion. Examples include passwords~\cite{password1979}, one-time passwords (OTPs)~\cite{otp2017} and physical devices~\cite{FIDO2021}. While such credentials are simple to verify, they are vulnerable to various attacks, such as password leaks, account recovery attacks, and device theft.
Consequently, biometric credentials are increasingly adopted in modern user devices, with access to most smartphones relying on face or fingerprint verification~\cite{most_smartphones_biometric}.

Previous work~$(\S\ref{sec:2})$ has conducted extensive analysis of biometric credentials in isolation, primarily focusing on reducing the \emph{false acceptance}~(\FAR) and \emph{false rejection}~(\FRR) \emph{rates}~\cite{Anil1997,prabhakar2003,Cappelli2006}. Sarkar et al.~\cite{sarkar2020} introduce the Equal Error Rate (\EER, where~$\FAR = \FRR$), a heuristic threshold of biometric sensors. While these works widely cover biometric credentials, none of them analyze their integration into an authentication mechanism. Other works analyze authentication mechanisms as predicates of credential possession~\cite{maram2024,mouallem2024,Eyal2021,Abhishek2013}, where each credential is treated as atomic. But they neglect the potential due to the special properties of biometric credentials.

In this work, we study how to design mechanisms with biometric credentials. We analyze how biometric credentials influence the authentication mechanism's success with and without atomic credentials. First, we present a refined definition of the authentication problem with biometric credentials~$(\S\ref{sec:3})$. The system comprises an authentication mechanism and two players: a~\emph{user} and an~\emph{attacker}. Both players try to satisfy the mechanism by providing a set of credentials.

Biometric credentials are evaluated by a sensor. The biometric sensors obtains a~\emph{sample}, calculates a similarity score between that sample and a~\emph{reference} stored in advance, and outputs~\emph{accept} if the score is above a set~\emph{match threshold} or \emph{reject} otherwise. A \emph{genuine distribution}~$f_U(s)$ denotes the score distribution of samples that belong to the legitimate user, whereas the \emph{impostor distribution}~$f_A(s)$ denotes the score distribution of samples that belong to the attacker, as shown in Figure~\ref{fig:intro-fig}. These distributions establish two error metrics known as~\FAR and~\FRR. 
...
To evaluate a mechanism comprising two biometric credentials, we consider first and second order approximations and use a numerical optimization algorithm to find the optimal thresholds for each biometric credential. Our results show that the optimal thresholds of the biometric credentials in such a mechanism are not, in general, the same as the optimal thresholds if used individually. This difference arises as we optimize each credential with respect to the entire mechanism, and not only with respect to the credential itself.
...
We then turn to analyze mechanisms that integrate a biometric and an atomic credential. Trying to identify the impact of an atomic credential on the optimal match threshold, we derive closed-form expressions for the mechanism's success probability. We symbolically calculate the optimal and~\EER thresholds and find that the optimum depends on the values of the atomic credential. These expressions reveal that the mechanism's success can primarily depend on either an atomic credential or a biometric credential, depending on the atomic credential probability values. For example, in an \emph{AND mechanism}, the mechanism grants access to a user if and only if all credentials are available to the user and at least one credential is unavailable to the attacker. The derived relation for this mechanism shows that when the atomic credential has a high probability of being safe, the mechanism’s success probability depends primarily on the atomic credential. In this case, even biometric samples that closely match the user reference have only a limited impact on the overall mechanism success probability; conversely, for other probability values of an atomic credential, success may depend more strongly on the biometric credential.

Motivated by this observation, we analyze whether the integration of a biometric credential with an atomic credential will always enhance the mechanism's success probability~$(\S\ref{sec:6})$. First, we link the biometric credential to the improvement it provides to the mechanism's success probability when it comprises an atomic credential. This link shows that when an atomic credential has zero \emph{loss} or \emph{leak} state probabilities, it is not worthwhile to integrate, as the atomic credential mechanism dominates the mechanism success probability. Then, we characterize the conditions under which integrating a biometric and an atomic credential improves the mechanism's success probability. 

We introduce a necessary and sufficient condition for an \emph{asymptotically separable} biometric credential: for every score above or below a match threshold, the error rates converge to zero. We prove that every \emph{asymptotically separable} biometric credential integrated with an atomic credential that has non-zero probabilities for the states of \emph{safe} and either \emph{loss} or \emph{leak}, improves the mechanism success probability. However, biometric credentials often exhibit a more common and weaker behavior~\cite{Slob2005,anil2004}. Thus, we introduce \emph{asymptotic dominance}: where the genuine distribution dominates the impostor distribution by a factor~$c$ for every score above a specific threshold, or the impostor dominates the genuine by a factor~$c$ for every score below a specific threshold. We show that this condition is sufficient for a biometric credential to improve the mechanism's success probability when integrated with an atomic credential.

We quantitatively demonstrate our theoretical analysis using public biometric data and a standard algorithm for fingerprint verification~\cite{sourceafis}~$(\S\ref{sec:7})$. We use a dataset of fingerprints from multiple users and evaluate matching scores by comparing samples of one user against those of other users. When calculating the matching score produced when different players attempt to authenticate as one another, we observe that the algorithm effectively extracts minutiae and the resulting scores are low. This results in a low~\EER value and a small overlapping area between the user and the attacker distributions. Thus, no significant improvement is observed when comparing the mechanism's success under the~\EER threshold and the optimal threshold. This indicates that the~\EER threshold may be sufficient and satisfactory when different users try to authenticate as one another.
