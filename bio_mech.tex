...
We examine distribution functions that approximate a biometric credential score distribution and characterize each player by their score distribution.
...
We seek to identify optimal mechanisms. Thus, we start by optimizing a single biometric credential mechanism~$(\S\ref{sec:4})$. We calculate~\FAR and~\FRR metrics for each biometric credential approximation and define its state probabilities. Then, we derive closed-form expressions of the mechanism's success to identify the optimal threshold of the biometric credential. We then symbolically calculate the optimal and~\EER thresholds and show that in general, the optimal threshold differs from the commonly used~\EER threshold. To understand the potential improvement of deviating from the~\EER, we link the likelihood of an attacker producing higher matching scores to the gap between the maximum success and the success at~\EER. Our findings reveal that the more likely an attacker is to produce higher matching scores~$s$, and the more asymmetric the user and attacker distributions become, the greater the benefit of using the optimal threshold instead of~\EER.
