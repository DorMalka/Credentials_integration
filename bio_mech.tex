...
We follow the same methodology as before and use the $\text{SourceAFIS}$ algorithm~\cite{sourceafis}. As reference images, we randomly select six Digital Persona scanner images of user 002\_0, while the probe set of a user comprises the remaining real and fake images. The geniune comparisons are all comparisons between the reference and real images in the probe set.
...
\begin{table*}[t]
    ...
    \begin{tabular}{cccccccccc}
        ...
        \midrule
        \textbf{0.55} & 0.40 & 0.04 & 0.01 & \textbf{0.478} & 0.721 & 0.575 & \textbf{0.807} & 30.64\% \\
        \textbf{0.63} & 0.04 & 0.32 & 0.01 & 0.733 & \textbf{0.544} & \textbf{0.836} & 0.655 & 38.79\% \\
        \textbf{0.72} & 0.04 & 0.23 & 0.01 & 0.746 & \textbf{0.618} & \textbf{0.868} & 0.745 & 47.91\% \\
        \textbf{0.81} & 0.14 & 0.04 & 0.01 & \textbf{0.692} & 0.760 & 0.835 & \textbf{0.899} & 58.25\% \\
        \textbf{0.90} & 0.05 & 0.04 & 0.01 & \textbf{0.766} & 0.773 & 0.925 & \textbf{0.932} & 70.03\% \\
        \bottomrule
    \end{tabular}
\end{table*}

We simulate a spoofing attack by using fake fingerprint images of the same user (002\_0) produced by~\textit{Gelatine}, \textit{Woodglue}, \textit{Latex} and~\textit{Ecoflex} and match them against the reference images to obtain the imposter score distribution. We build the resulting histogram of a user and an imposter (Figure~\ref{fig:livdet_sourceafis_histogram}) and then calculate the~\FAR-\FRR curve to compute the~\EER point (Figure~\ref{fig:livdet_sourceafis_far_frr}). As we expect, the~\EER increased significantly to 17.8\% indicating the user and imposter distributions are less separable and more overlapping in spoofing attacks. To understand the implications of a spoofing attack, we calculate the success probability of a mechanism uses a biometric credential and a mechanism uses an atomic credential and a biometric credential, as shown in Figure~\ref{fig:livdet_sourceafis_success}. We find that the optimal operating point reducing the failure probability between 30\% to 70\% compared to the~\EER point when the mechanism uses both an atomic and a biometric credential. Moreover, we find that a mechanism uses an atomic credential and a biometric credential at the~\EER threshold, might achieve lower success probability than that of a mechanism uses an atomic credential only, or a biometric credential only. These results are summarized in Table~\ref   {tab:psafe_sweep_improvement}.

\section{Conclusion}\label{sec:8}
\noindent
We find that the optimal threshold of a biometric credential depends on the other credentials in the mechanism and differs from the heuristic threshold commonly used today. We characterize a condition on the biometric credential and prove that whenever this condition holds, a mechanism designer always benefits from adding a biometric credential over using atomic credentials alone. We quantify this theoretical result by simulating different scenarios using public biometric datasets, including spoofing attacks. Our results show that optimizing the threshold significantly reduces the failure probability under spoofing attacks by~30-70\%.
