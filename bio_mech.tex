...
We now evaluate mechanisms that use atomic credentials and biometric credential derived from real-world public~\textit{FVC2004} dataset of fingerprints~\cite{fvc2004}. We use a standard fingerprint verification algorithm~\textit{SourceAFIS}~\cite{sourceafis} for scoring fingerprints out of their images based on minutiae triplets. We evaluate multiple users from~$\textit{FVC2004}$ \textit{Database1}. For each user, we randomly select three fingerprint templates as probe templates of and compare them with the remaining templates in the database. For each candidate scan, we compute its similarity score by running the algorithm against each of the three probes and choose the maximum score as the candidate final score. For example, if the matching scores between a candidate scan and the three probes are~$[0.1,0.6,0.9]$, then the resulting score for this candidate is~$0.9$. The geniune combinations are defined as all combinations between the user probes and the other users' scans appears in the database. Alternatively, the imposter combinations are defined as all the other combinations between the user probes and the other users' scans.

Like Jain et al.~\cite{Anil1997}, Cappelli et al.~\cite{Cappelli2006} and Slobodan et al.~\cite{Slob2005}, we produce histograms of the geniune and imposter combinations as shown in Figure~\ref{fig:sourceafis_histogram}. Then, we turn the histogram into a smoothed probability density functions and calculate the~\FAR and~\FRR as functions of the threshold~$t$, Figure~\ref{fig:sourceafis_far_frr}. This process is consistent with Slobodan et al.~\cite{Slob2005} and Cappelli et al.~\cite{Cappelli2006} presented in their experimental results.

The most important outcome of this approach was achieving an~\EER of approximately~2\% similar to the best results reported in the competition~\cite{fvc2004} and appears also at industry and academical level sensors when evaluating different users~\cite{Maio2002,Cappelli2006}. This allows us to objectively compare our results with the state-of-the-art fingerprint verification algorithms. We calculate the mechanisms successs probability (Equation~\ref{eqn:safesuccess}) of a mechanism uses a biometric credential only and show that the optimal operating point is different from the~\EER point and achieves higher success probability Figure~\ref{fig:bio_only_sourceafis_reg_scoring}. Finally, we illustrate the success probability function of a mechanism uses an atomic credential with arbitrary fault probabilities~$P^{1}_\text{safe} = 0.75$ and~$P^{1}_\text{leak} = 0.1$, $P^{1}_\text{loss} = 0.1$ and $P^{1}_\text{stolen} = 0.05$ and the biometric credential Figure~\ref{fig:bio_atm_sourceafis_reg_scoring}.
...
We show that the optimal operating point of the mechanism uses both an atomic credential and a biometric credential is different from the~\EER point and achieves higher success probability by approximately~2\% compared to the~\EER point. In addition, the optimal point in both \emph{AND mechanism} and \emph{OR mechanism} achieves higher success probability than the atomic credetial only mechanism as observed in Figure~\ref{fig:bio_atm_sourceafis_reg_scoring}. However, the success probability of the mechanism uses both an atomic credential and a biometric credential which operates in optimal threshold is not significantly greater then the one operates at the~\EER point. This can be explained by the fact that the~\EER point of the biometric credential is already very low, indicating high seperability between a user and an attacker and thus, there is no much room for improvement. In that scenario, we tested different users against each other. However, our main approach in this paper was to increase the mechanism's success in scenarios where an imposter will try to spoof the user templates and authenticate into the user account. Hence, we expect the imposter to use spoofing tools, producing samples resamble with score distribution function that much more similar to the genuine distribution function, which might lead to an increased~\EER point. 
...
For each candidate scan, we define the similarity score as the maximum score among 6 probe templates, similarly to the previous simulation. 
\begin{table*}[t]
    \centering
...
However, current biometric sensors prevent the threshold selection from mechanism designers and integrators, achieving a suboptimal mechanism success probability. This paper provides the foundation for developing new authentication protocols based on threshold selection. We further propose that biometric sensor vendors should change their interface and let the integrators set the thresholds of the biometric credentials. Harnessing our implication may enhance the security of digital and physical assets for millions of users around the world.
