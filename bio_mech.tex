...
\subsection{Spoofing Attack Scenario}\label{sec:7.2}
\noindent
...
\noindent
We simulate a spoofing attack scenario in which an attacker has multiple attempts to authenticate, using either the same or different spoofed fingerprints. We use the same genuine and impostor distributions calculated in the previous section~\S\ref{sec:7.2} using the LivDet2015 database. 

We follow the theoretical analysis in~\S\ref{sec:4.2}, and calculate the success probability of a mechanism that uses a biometric credential only~(Equation~\ref{eq:attempts_success}). Then, we simulate several authentication attempts, each one produces different success probability function. We comapre the mechanism success probability under three different thresholds selection: (1)~the~\EER threshold reused for all~$k$ attempts, (2)~the one attempt optimal threshold reused for all~$k$ attempts, and (3)~threshold optinization over all~$k$ attempts, as shown in Figure~\ref{fig:livdet_sourceafis_success_multiple_attempts}. 
...
% FAR at the final attempt
% \addplot[red, mark=square*]
% table[x=k, y=FAR] 
% {figs/fig_spoofed_users/livdet_fixed_prefix_far_frr_success_by_k.txt};
% \addlegendentry{FAR}
% FRR at the final attempt
...
table[x=k, y=P_success_max] 
{figs/fig_spoofed_users/livdet_fixed_prefix_far_frr_success_by_k.txt};
\addlegendentry{$P_{\mathrm{success}}^{\max}$}
\addplot[orange, mark=diamond*]
table[x=k, y=product_FRR] 
{figs/fig_spoofed_users/livdet_fixed_prefix_far_frr_success_by_k.txt};
\addlegendentry{$\prod_{j=1}^{k}\mathrm{FRR}_j$}
\addplot[purple, mark=pentagon*]
table[x=k, y=product_1_minus_FAR] 
{figs/fig_spoofed_users/livdet_fixed_prefix_far_frr_success_by_k.txt};
\addlegendentry{$\prod_{j=1}^{k}(1-\mathrm{FAR}_j)$}
\end{axis}
\end{tikzpicture}
\caption{FAR, FRR, and the maximal cumulative success probability as a
function of the number of authentication attempts~$k$. FAR and FRR correspond
to the threshold applied at the final attempt of each fixed-prefix policy.}
\label{fig:fixed_prefix_far_frr_success}
\end{figure}
...
% % ==================================================
% % Uniform distributions: solid lines
% % ==================================================

% \addplot[green!60!black, solid, mark=*] 
% table[x=k, y=P_success_max] 
% {figs/fig_theoretical_k_attempts/uniform_k_attempts.txt};
% \addlegendentry{Uniform $P_{\mathrm{success}}^{\max}$}

% \addplot[orange, solid, mark=*] 
% table[x=k, y=product_FRR] 
% {figs/fig_theoretical_k_attempts/uniform_k_attempts.txt};
% \addlegendentry{Uniform $\prod_{j=1}^{k}\mathrm{FRR}_j$}

% \addplot[purple, solid, mark=*] 
% table[x=k, y=product_1_minus_FAR] 
% {figs/fig_theoretical_k_attempts/uniform_k_attempts.txt};
% \addlegendentry{Uniform $\prod_{j=1}^{k}(1-\mathrm{FAR}_j)$}

% % ==================================================
% % Quadratic distributions: dashed lines
% % ==================================================

% % \addplot[green!60!black, dashed, mark=square*] 
% % table[x=k, y=P_success_max] 
% % {figs/fig_theoretical_k_attempts/quadratic_k_attempts.txt};
% % \addlegendentry{Quadratic $P_{\mathrm{success}}^{\max}$}

% % \addplot[orange, dashed, mark=square*] 
% % table[x=k, y=product_FRR] 
% % {figs/fig_theoretical_k_attempts/quadratic_k_attempts.txt};
% % \addlegendentry{Quadratic $\prod_{j=1}^{k}\mathrm{FRR}_j$}

% % \addplot[purple, dashed, mark=square*] 
% % table[x=k, y=product_1_minus_FAR] 
% % {figs/fig_theoretical_k_attempts/quadratic_k_attempts.txt};
% % \addlegendentry{Quadratic $\prod_{j=1}^{k}(1-\mathrm{FAR}_j)$}

% % ==================================================
% % Gaussian distributions: dotted lines
% % ==================================================

% % \addplot[green!60!black, densely dotted, mark=triangle*] 
% % table[x=k, y=P_success_max] 
% % {figs/fig_theoretical_k_attempts/gaussian_k_attempts.txt};
% % \addlegendentry{Gaussian $P_{\mathrm{success}}^{\max}$}

% % \addplot[orange, densely dotted, mark=triangle*] 
% % table[x=k, y=product_FRR] 
% % {figs/fig_theoretical_k_attempts/gaussian_k_attempts.txt};
% % \addlegendentry{Gaussian $\prod_{j=1}^{k}\mathrm{FRR}_j$}

% % \addplot[purple, densely dotted, mark=triangle*] 
% % table[x=k, y=product_1_minus_FAR] 
% % {figs/fig_theoretical_k_attempts/gaussian_k_attempts.txt};
% % \addlegendentry{Gaussian $\prod_{j=1}^{k}(1-\mathrm{FAR}_j)$}

% \end{axis}
% \end{tikzpicture}
% \caption{The maximal success probability, the product of the false rejection
% rates, and the product of the attacker rejection probabilities as functions
% of the number of authentication attempts~$k$ for the uniform, quadratic, and
% Gaussian score distributions.}
% \label{fig:theoretical_k_attempts_comparison}
% \end{figure}
...
% % \addlegendentry{Uniform $P_{\mathrm{failure}}$}
...
% \addlegendentry{Quadratic $P_{\mathrm{failure}}$}
...
% % \addlegendentry{Gaussian $P_{\mathrm{failure}}$}
