...
\addplot[blue, thick, const plot mark right, no marks]
table[x=score, y=genuine_cdf, col sep=space] 
{figs/fig_spoofed_users/livdet_sourceafis_empirical_cdf_data.txt};
\addlegendentry{Genuine}
\addplot[red, thick, const plot mark right, no marks]
table[x=score, y=impostor_cdf, col sep=space] 
{figs/fig_spoofed_users/livdet_sourceafis_empirical_cdf_data.txt};
\addlegendentry{Impostor}
\end{axis}
\end{tikzpicture}
\caption{Empirical cumulative distributions of the genuine and spoof-impostor similarity scores.}
\label{fig:livdet_empirical_cdf}
\end{figure}
...
\begin{axis}[
    ...
    xlabel={Normalized similarity threshold},
    ylabel={Success probability},
    xmin=0,
    xmax=100,
    ymin=0,
...
\addplot[blue,thick,no markers]
table[x=threshold,y=success_k4,col sep=space] 
{figs/fig_spoofed_users/livdet_attempts_success_k4.txt};
\addlegendentry{$k=4$ attempts}
\end{axis}
\end{tikzpicture}
\caption{Biometric-only success probability over four authentication
attempts when the same threshold is used in every attempt.}
\label{fig:livdet_sourceafis_success_multiple_attempts}
\end{figure}
