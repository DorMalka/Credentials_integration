...
\subsection{Spoofing Attack With Multiple Attempts}
\noindent
...
We next consider a different policy where the user is allowed~$k_u$ authentication attempts and the attacker is allowed~$k_a$ spoofing attempts. We use the~\FAR and~\FRR estimated from the LivDet2015 database in previous section~\S\ref{sec:7.2}, and find the optimal number of attempts for a user to successfully authenticate, and the maximal number of allowed attempts for an attacker to successfully spoof the biometric credential. We then calculate the success probability of a mechanism that uses a biometric credential only, when the user has~$k_u$ attempts and the attacker has~$k_a$ attempts, as shown in Figure~\ref{fig:Success_probability_by_k_user_attacker}. Our findings show that 

We next consider a different policy where we fix the number of attempts for the attacker to~$k_a=10$ and the number of attempts for the user to~$k_u=3$. These selections simulate a real-world scenario that represents the typical number of attempts for a user authentication versus the maximal allowed attempts forced by the mechanism~\cite{typical_aut_attempts}. We use the~\FAR and~\FRR estimated from the LivDet2015 database in previous section~\S\ref{sec:7.2} and examine how the threshold selection affects the success probability of a mechanism that uses a biometric credential only. Under this policy, we present the success probability as function of the number of attempts in three baselines: (1)~the~\EER threshold reused for all~$k$ attempts, (2)~the one attempt optimal threshold reused for all~$k$ attempts, (3)~threshold optimization over all~$k$ attempts, as shown in Figure~\ref{fig:fixed_user_attacker_success_by_k}. We find that for the first three attempts, the optimal threshold doesn't change and is optimized repectively for~$k_u$. That means different user attempts lead to different optimal thresholds over~$k_u$. However, as the attempt index increases over~$k_u$, the threshold increases to reduce the probability of a successful spoofing attack. We also find that the success probability of a mechanism that uses a biometric credential only, is significantly higher when using the optimal threshold selection over~$k_u$ attempts, compared to reusing the~\EER threshold for all~$k_u$ attempts. 
