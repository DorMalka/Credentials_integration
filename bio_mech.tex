...
\noindent
We will now characterize the conditions under which integrating a biometric credential into a single atomic credential mechanism, enhances its success. We show that for any mechanism with a single atomic credential characterized by non-zero probabilities~$ P^1_\text{safe} \text{ and } P^1_\text{stolen}$ only, integrating a biometric credential does not improve mechanism's success probability~$(\S\ref{sec:6.1})$. Next, we present a necessary and sufficient condition for such improvement~$(\S\ref{sec:6.2})$ and derive a more common criterion that provides a sufficient condition~$(\S\ref{sec:6.3})$.
\subsection{No Improvement In Mechanisms Success}\label{sec:6.1}
\noindent
We first present a condition under which adding a biometric credential into the mechanism~$M$ cannot produce a new mechanism~$\bar{M}$ whose success probability is higher than that of the atomic credential alone.
\begin{proposition}
    Let~$M$ be a mechanism comprising an atomic credential with~$P^1_\text{loss} = P^1_\text{leak} = 0$. Then for every mechanism~$\bar{M}$ obtained by adding a biometric credential to~$M$,
    \begin{equation*}
        P_{\mathrm{success}}^{\bar{M}} < P_{\mathrm{success}}^{M}.
    \end{equation*}
\end{proposition}
\begin{proof}
    The mechanism~$\bar{M}$ comprises one atomic credential and one biometric credential in two possible compositions~(see~$\S\ref{sec:5}$). An \emph{AND mechanism} has success probability described by Equation~\ref{eqn:andsuccess}, and an \emph{OR mechanism} has success probability described by Equation~\ref{eqn:orsuccess}. For both equations,~$P^2_{\text{safe}}(t)$ denotes the safe probability of the biometric credential as a function of the operating threshold~$t$. Since the state probabilities of the atomic credential satisfy~$ P^1_\text{leak} =  P^1_\text{loss} = 0$, the success probability of~$\bar{M}$ reduces in both cases to
    \begin{equation*}
        P_{\text{success}}(t) = P^1_\text{safe} \cdot P^2_{\text{safe}}(t).
    \end{equation*}
    Each state probability of the biometric credential is strictly lower than one; in particular,~$P^2_{\text{safe}}(t) < 1$ for every value of $t$. Therefore,~$P_{\text{success}}(t) \le P^1_\text{safe}$ and thus, adding a biometric credential does not produce a new mechanism~$\bar{M}$ whose success probability is greater than that of a single atomic credential mechanism.
\end{proof}
...
We now identify the conditions under which adding a biometric credential to a single atomic credential mechanism~$M$, can produce a new mechanism~$\bar{M}$ with a greater success probabiltiy.
We consider an atomic credential with non-zero probabilities~$ P^1_\text{safe}$ and at least one of~$P^1_\text{leak}$ $\text{or } P^1_\text{loss}$. We define the condition of \emph{asymptotic separability} for a biometric credential and show that it is a necessary and sufficient condition for adding the biometric credential to produce a new mechanism whose success probability is greater than that of a single atomic credential mechanism.
\begin{definition}
...
\begin{proposition}\label{thm:suf_nec_cond_for_improve}
    Let~$M$ be a mechanism comprising an atomic credential with~$P^1_\text{safe} > 0$ and at least one of~$P^1_\text{leak},P^1_\text{loss} > 0$. Then there exists~$\bar{M}$ obtained by adding a biometric credential whose user and attacker distributions are asymptotically separable to~$M$, such that
    \begin{equation*}
        P_{\mathrm{success}}^{\bar{M}} > P_{\mathrm{success}}^{M}
    \end{equation*}
\end{proposition}
...
\begin{proof}
    ...
    \contprobabilities
    Integrating a biometric credential into~$M$, which comprises an atomic credential under the given settings, produces a new mechanism~$\bar{M}$ in two possible compositions: an \emph{AND mechanism} with success probability described by Equation~\ref{eqn:andsuccess}, and an \emph{OR mechanism} with success probability described by Equation~\ref{eqn:orsuccess}. Thus, by placing the biometric credential in each configuration we can express the success probability of~$\bar{M}$ as a function of the operating threshold~$t$
    \begin{equation}
        \label{eqn:andineq}
    \ANDintegration
    \end{equation}
    ...
    By algebraic manipulation of Equation~\ref{eqn:andineq}~$P^{\text{AND}}_{\text{success}}(t) > P^1_{\text{safe}}$ if and only if:
    \begin{equation*}
    ...
    \end{equation*}
    \begin{equation}\label{eqn:necessary term_1}
    \frac{\FRR(t)}{1-\FAR(t)} < \frac{P^{1}_{\text{leak}}}{P^{1}_{\text{safe}}+P^{1}_{\text{leak}}\cdot(1-\FAR(t))}
    \end{equation}
    Division by~$(1-\FAR(t))$ is valid under the imposed constraint of~$P^{2}_\textit{safe}>0$, which implies~$\FAR<1$ and~$\FRR<1$.
    ...
    \end{equation}
    Division by~$(1-\FRR(t))$ is valid under the same imposed constraint of~$P^{2}_\textit{safe}>0$.
    ...
    = 0.
    \end{equation*}
    Equivalently,~$\forall \varepsilon > 0, \exists t$ such that either~$\frac{\FAR(t)}{1-\FRR(t)} < \varepsilon$ or~$\frac{\FRR(t)}{1-\FAR(t)} < \varepsilon$. Thus, we can choose $\varepsilon$ that equals the right hand side of either Equation~\ref{eqn:FRRineq} or Equation~\ref{eqn:FARineq} and by definition we know that a~$t$ satisfies the inequality exists. This existance of~$t$ does not break the constraint~$P^{2}_\textit{safe}>0$ since neither~$\FAR=1$ nor~$\FRR=1$ to satisfy the asymptotic separability condition. More precisely, if \[\inf_{t}\frac{\FAR(t)}{1-\FRR(t)} = 0,\] 
...
\begin{proposition}
    Let~$\bar{M}$ be a mechanism comprising a biometric credential and an atomic credential with~$P_\text{safe}>0$ and at least one of~$P_\text{leak},P_\text{loss} > 0$. Suppose there exists~$t$ such that
    \begin{equation*}
...
\end{proposition} 
\begin{proof}
    Assume by contradiction that the biometric credential is not asymptotically separable. Then there exists~$\varepsilon > 0$ such that for all~$t$ both~$\frac{\FAR(t)}{1-\FRR(t)} \ge \varepsilon$ and~$\frac{\FRR(t)}{1-\FAR(t)} \ge \varepsilon$. Since it is given that the integrated mechanism~$\bar{M}$ achieves a strictly greater success probability than an atomic credential mechanism~$M$, then there exists a threshold $t$ such that either:~$P^{\text{AND}}_{\text{success}}(t) > P^1_{\text{safe}} \textit{ or }$ $P^{\text{OR}}_{\text{success}}(t) > P^1_{\text{safe}}.$ Thus, for every value of~$P^{1}_{\text{safe}}, P^{1}_{\text{leak}}, P^{1}_{\text{loss}}$ in the range~$(0,1)$ necessarily either Equation~\ref{eqn:necessary term_1} or Equation~\ref{eqn:necessary term_2} holds. In particular, for a mechanism~$\bar{M}$ comprising an atomic credential with state probabilities~$P^{1}_{\text{safe}}, P^{1}_{\text{leak}}, P^{1}_{\text{loss}}$ such that either
    \begin{equation*}
        \begin{aligned}
        &\frac{P^{1}_{\text{leak}}}{P^{1}_{\text{safe}}+P^{1}_{\text{leak}}\cdot(1-\FAR(t))} < \frac{P^{1}_{\text{leak}}}{P^{1}_{\text{safe}}} \textit{, or }\\
        &\frac{P^{1}_{\text{loss}}}{P^{1}_{\text{safe}}+P^{1}_{\text{loss}}\cdot(1-\FRR(t))} < \frac{P^{1}_{\text{loss}}}{P^{1}_{\text{safe}}} \textit{, and }\\
        &\varepsilon = \max\left\{\frac{P^{1}_{\text{leak}}}{P^{1}_{\text{safe}}}, \frac{P^{1}_{\text{loss}}}{P^{1}_{\text{safe}}}\right\}.
        \end{aligned}
    \end{equation*} 
    By the contradiction assumption that the biometric credential is not asymptotically separable it follows that for every threshold~$t$
    \begin{equation*}
        \begin{aligned}
            &\frac{\FRR(t)}{1-\FAR(t)} \ge \frac{P^{1}_{\text{leak}}}{P^{1}_{\text{safe}}} \textit{ and } \\
            &\frac{\FAR(t)}{1-\FRR(t)} \ge \frac{P^{1}_{\text{loss}}}{P^{1}_{\text{safe}}}.
        \end{aligned}
    \end{equation*} 
    This contradicts the necessary condition that the success probability of~$\bar{M}$ is strictly greater than that of~$M$ presented in both Equation~\ref{eqn:necessary term_1} and Equation~\ref{eqn:necessary term_2}. Therefore, the biometric credential must be asymptotically separable.
\end{proof}
...
Let~$M$ be a mechanism comprising an atomic credential with~$P_\text{safe}>0$ and at least one of~$P_\text{leak},P_\text{loss} > 0$. Then there exists~$\bar{M}$ obtained by adding a biometric credential whose user and attaker distributions are asymptotically dominated to~$M$, such that 
\begin{equation*}
    P^{\bar{M}}_{\text{success}}(t) \ge P^{M}_{\text{success}} .
\end{equation*}
\end{proposition}
...
\end{equation*}
This is true~$\forall c > 0,$ thus we can choose~$c$ that equals the right hand side of~Equation~\ref{eqn:FRRineq} and satisfies the sufficient condition.
...
\end{equation*}
We showed that~$\forall c>0$  either~$\frac{\FAR(t)}{1-\FRR(t)} < c$ or $\frac{\FRR(t)}{1-\FAR(t)} < c$. Therefore, integrating an asymptotically dominated biometric credential with an atomic credential produces~$\bar{M}$ with greater mechanism success probability.
\end{proof}
