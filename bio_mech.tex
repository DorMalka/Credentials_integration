...
\noindent
We consider a mechanism that initially consists of a single atomic credential. We then examine whether adding a biometric credential into this mechanism can produce a new mechanism whose success probability is higher than that of the atomic credential alone.

\begin{proposition}
    Let~$M$ be a mechanism comprising an atomic credential with non-zero probabilities~$P^1_\text{safe}$ and~$P^1_\text{stolen}$ only. Then, adding a biometric credential to~$M$ cannot produce a new mechanism~$\bar{M}$ whose success probability is higher than that of~$M$.
\end{proposition}
\begin{proof}
    Adding a biometric credential into a mechanism~$M$ comprising an atomic credential with non-zero probabilities~$ P^1_\text{safe}$ and~$ P^1_\text{stolen}$ only, produces a new mechanism~$\bar{M}$. This new mechanism~$\bar{M}$ comprises one atomic credential and one biometric credential in two possible compositions~(see~$\S\ref{sec:5}$). An \emph{AND mechanism} has success probability described by Equation~\ref{eqn:andsuccess}, and an \emph{OR mechanism} has success probability described by Equation~\ref{eqn:orsuccess}. Since the state probabilities of the atomic credential are~$ P^1_\text{leak} =  P^1_\text{loss} = 0$, the success probability of~$\bar{M}$ reduces in both cases to
    \begin{equation*}
        P_{\text{success}}(t) = P^1_\text{safe} \cdot P^2_{\text{safe}}(t).
    \end{equation*}
    Here,~$P^2_{\text{safe}}(t)$ denotes the safe probability of the biometric credential as a function of the operating threshold $t$.
    Each state probability of the biometric credential is strictly lower or equal to one; in particular,~$P^2_{\text{safe}}(t) \le 1$ for every value of $t$. Therefore,~$P_{\text{success}}(t) \le P^1_\text{safe}$ and thus, adding a biometric credential does not produce a new mechanism whose success probability is higher than that of of a single atomic credential mechanism.
\end{proof}
\subsection{Sufficient And Necessary Condition for Integrating Biometric Credentials}\label{sec:6.2}
\noindent
%We have shown that adding a biometric credential to a mechanism comprising an atomic credential with non-zero probabilities~$ P^1_\text{safe}$ and~$ P^1_\text{stolen}$ only, cannot produce a new mechanism with greater success probability. 
We now identify the conditions under which adding a biometric credential to a single atomic credential mechanism, can produce a new mechanism with a higher success probabiltiy.
We consider an atomic credential with non-zero probabilities~$ P^1_\text{safe}$ and at least one of~$P^1_\text{leak}$ $\text{or } P^1_\text{loss}$. We define the condition of \emph{asymptotic separability} for a biometric credential and show that it is a necessary and sufficient condition for adding the biometric credential to produce a new mechanism whose success probability is higher than that of a single atomic credential mechanism.
\begin{definition}
A biometric credential with user distribution function~$f_U(s)$ and attacker distribution function~$f_A(s)$ produces~\FAR and~\FRR that equal 
\begin{equation*}
    \begin{aligned}
    &\FAR(t) = \int_{t}^{\infty} f_A(s) ds \textit{ and,} \\
    &\FRR(t) = \int_{-\infty}^{t} f_U(s) ds.
    \end{aligned}
\end{equation*}
Therefore the user distribution and attacker distribution are \emph{asymptotically separable} if
\begin{equation*}
    \inf_{t}
    \min\!\left\{
    \frac{\FAR(t)}{1-\FRR(t)},\,
...
\begin{proposition}\label{thm:suf_nec_cond_for_improve}
    Let~$M$ be a mechanism comprising an atomic credential with~$P^1_\text{safe} > 0$ and at least one of~$P^1_\text{leak},P^1_\text{loss} > 0$. Then, adding a biometric credential whose user and attacker distributions are asymptotically separable, produces a new mechanism~$\bar{M}$ whose success probability is higher than that of~$M$.
\end{proposition}

\begin{proof}
    The success probability of a mechanism comprising an atomic credential only is given by Equation~\ref{eqn:safesuccess}. A biometric credential is characterized by four state probabilities, that are functions of~$t$: $P^{2}_{\text{safe}}(t)$, $P^{2}_{\text{loss}}(t)$, $P^{2}_{\text{leak}}(t)$, $P^{2}_{\text{theft}}(t)$. We can describe each one of these probabilities as a function of~\FAR and~\FRR as:
    \contprobabilities
    Adding a biometric credential to a mechanism~$M$ comprising an atomic credential under the given settings, produces a new mechanism~$\bar{M}$ in two possible compositions.
    The success probability described by Equation~\ref{eqn:andsuccess} refers to \emph{AND mechanism}, while the success probability described by Equation~\ref{eqn:orsuccess} refers to \emph{OR mechanism}. Thus, by placing the biometric credential in each configuration we can express the success probability of~$\bar{M}$ as a function of the operating threshold $t$
    \begin{equation}
        \label{eqn:andineq}
    \ANDintegration
    \end{equation}
    ...
    \ORintegration
    \end{equation}
    To prove that the integration of a biometric credential produces a mechanism~$\bar{M}$ with higher success probability than that of~$M$, we need to prove that there exists an operating threshold $t$ such that either 
    ...
    then the \textsc{OR} mechanism composition is preferable, whereas if \[\inf_{t}\frac{\FRR(t)}{1-\FAR} = 0,\] then the \textsc{AND} mechanism composition is preferable. Therefore, adding an asymptotically separable biometric credential to~$M$ produces a new mechanism~$\bar{M}$ with higher success probability.
\end{proof}

\begin{proposition}
Suppose there exists~$t$ such that the integrated mechanism~$\bar{M}$ comprises a biometric credential and an atomic credential with~$P_\text{safe}>0$ and at least one of~$P_\text{leak},P_\text{loss} > 0$, then~$\bar{M}$ achieves a strictly higher success probability than~$M$, i.e.,
\begin{equation*}
P^{\text{integrated}}_{\text{success}}(t) > P^{\text{atomic}}_{\text{success}}.
\end{equation*}
Then the biometric credential is asymptotically separable.
\end{proposition} 
\begin{proof}
    Assume by contradiction that the biometric credential is not asymptotically separable. Then $\exists \varepsilon > 0$ such that $\forall t$ both $\frac{\FAR(t)}{1-\FRR(t)} \ge \varepsilon$ and $\frac{\FRR(t)}{1-\FAR(t)} \ge \varepsilon$. Since it is given that the integrated mechanism~$\bar{M}$ achieves a strictly higher success probability than an atomic credential mechanism~$M$, then there exists a threshold $t$ such that either:~$P^{\text{AND}}_{\text{success}}(t) > P^1_{\text{safe}} \textit{ or }$ $P^{\text{OR}}_{\text{success}}(t) > P^1_{\text{safe}}.$ Thus, either Equation~\ref{eqn:FRRineq} or Equation~\ref{eqn:FARineq} holds for every value of~$P^{1}_{\text{safe}}, P^{1}_{\text{leak}}, P^{1}_{\text{loss}}$ in the range~$(0,1)$. In particular, we can choose~$P^{1}_{\text{safe}}, P^{1}_{\text{leak}}, P^{1}_{\text{loss}}$ such that either
...
\noindent
Since biometric credentials often exhibit a more common behavior~\cite{Slob2005,anil2004}, we introduce an \emph{asymptotic dominancy} notion which captures this property. We prove that adding an \emph{asymptotically dominated} biometric credential can produce a new mechanism~$\bar{M}$ with higher success probability than that of a single atomic credential mechanism~$M$, and show it is a sufficient condition.
\begin{definition}
\label{def:User and Attacker distribution functions}
A biometric credential with a user distribution fucntion~$f_U(s)$ and an attacker distribution function~$f_A(s)$ are \emph{asymptotically dominated} if
\begin{equation*}
\begin{aligned}
...
Let~$M$ be a mechanism comprising an atomic credential with success probability~$P^{\text{atomic}}_{\text{success}}$. Then adding a biometric credential that is asymptotically dominated to~$M$ produces a new mechanism~$\bar{M}$ whose success probability is higher than that of~$M$, i.e.,
\begin{equation*}
    P^{\text{integrated}}_{\text{success}}(t) \ge P^{\text{atomic}}_{\text{success}} .
\end{equation*}
\end{proposition}
...
The success probability of a mechanism comprising an atomic credential only is given by Equation~\ref{eqn:safesuccess}. A biometric credential is characterized by four state probabilities, that are functions of~$t$: $P^{2}_{\text{safe}}(t)$, $P^{2}_{\text{loss}}(t)$, $P^{2}_{\text{leak}}(t)$, $P^{2}_{\text{theft}}(t)$. We can describe each one of these probabilities as a function of~\FAR and ~\FRR as:
\contprobabilities
Adding a biometric credential to~$M$ produces two mechanism compositions with success probabilities given by Equation~\ref{eqn:andsuccess} or Equation~\ref{eqn:orsuccess}. Thus, we can express the success probability of~$\bar{M}$ as a function of the operating threshold~$t$ as shown in Equation~\ref{eqn:andineq} and Equation~\ref{eqn:orineq}. We previously observed that a sufficient condition for producing a new mechanism with higher success is described by Equation~\ref{eqn:FRRineq} and Equation~\ref{eqn:FARineq}. Thus, we need to show that there exists an operating threshold $t$ such that either:
