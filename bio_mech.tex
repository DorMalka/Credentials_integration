...
\begin{proposition}\label{thm:suf_nec_cond_for_improve_k_atm}
    Let~$\{c_1,c_2,\ldots,c_k\}$ be a set of~$k$ independent atomic credentials with~$P^i_\text{safe} > 0$ and at least one of~$P^i_\text{leak},P^i_\text{loss} > 0$, $c_{k+1}$ be a biometric credential whose user distribution~$f_U(t)$ and attacker distribution~$f_A(t)$ are asymptotically separable and~$M$ be a mechanism using~$\{c_1,c_2,\ldots,c_k\}$. Then, there exists a mechanism~$\bar{M}$ using~$\{c_1,c_2,\ldots,c_k,c_{k+1}\}$ such that
    \begin{equation*}
        P_{\mathrm{success}}^{\bar{M}} > P_{\mathrm{success}}^{M}
    \end{equation*}
\end{proposition}
\begin{proof}
    Let~$M$ be a mechanism using~$\{c_1,c_2,\ldots,c_k\}$. The mechanism~$M$ induces a reduction of the~$k$ credentials into a single abstract credential. Since each atomic credential~$c_i$ has~$P^i_\text{safe} > 0$ and at least one of~$P^i_\text{leak}$, or~$P^i_\text{loss} > 0$, then the induced credential of~$M$ is a single atomic credential with state probabilities~$P^M_\text{safe}>0$, and at least one of~$P^M_\text{leak}$, or~$P^M_\text{loss} > 0$. By Proposition~\ref{thm:suf_nec_cond_for_improve}, there exists a mechanism~$\bar{M}$ using the induced credential and the biometric credential that achieves greater success probability than that of the induced credential mechanism. Since the induced credential is equivalent to the original mechanism~$M$, it follows that there exists a mechanism~$\bar{M}$ using~$\{c_1,c_2,\ldots,c_k,c_{k+1}\}$ that achieves greater success probability than that of~$M$.
\end{proof}
\begin{proposition}
