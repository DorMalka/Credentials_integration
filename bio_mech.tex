...
We now evaluate our framework on real-world public datasets based on fingerprints taken from~$\textit{FVC2002}$ database~\cite{fvc2002} and \textit{FVC2004} database~\cite{fvc2004}. We chose fingerprints databases, as it is a widely common and used example of a biometric credential in authentication mechanisms. We use a standard fingerprint verification algorithm~\textit{SourceAFIS} created by Vazan~\cite{sourceafis} for scoring fingerprints out of their images based on minutiae triplets. For a test set, we took five templates out of~$\textit{FVC2002}$ \textit{database1} of user 103 and compare them against the whole database of both~$\textit{FVC2002}$ and~$\textit{FVC2004}$. A geniune will be considered the templates of user 103 against its other templates while imposter considered as all the other combinations. For scoring methodology, we define the final score as the maximum similarity across all 5 user templates. For example, if the matching scores between a sample and the 5 probes are~$[0.1,0.3,0.6,0.9,0.4]$, then the resulting score is 0.9. 

We maintain same approach as Jain et al.~\cite{Anil1997}, Cappelli et al.~\cite{Cappelli2006} and Slobodan et al.~\cite{Slob2005} and build histograms of the geniune and imposter combinations as shown in Figure~\ref{fig:sourceafis_histogram}. Then, we turn the histogram into a smoothed probability density function to find the~\FAR and~\FRR Figure~\ref{fig:sourceafis_far_frr}. These findings are consistent with the results of Slobodan et al.~\cite{Slob2005} and Cappelli et al.~\cite{Cappelli2006} presented in their test results.

The most important outcome from this approach was to achieve similar~\EER of approximately 2\% as achieved at industry and academical level sensors when testing different users~\cite{Maio2002,Cappelli2006}, to objectively make a comparison with state-of-the-art algorithms. We calculate the mechanisms successs probability (Equation~\ref{eqn:safesuccess}) of a mechanism uses a biometric credential only and show that the optimal operating point is different from the~\EER point and achieves higher success probability Figure~\ref{fig:bio_only_sourceafis_reg_scoring}. Finally, we illustrate the success probability function of a mechanism uses an atomic credential with arbitrary fault probabilities~$P^{1}_\text{safe} = 0.7$ and~$P^{1}_\text{leak} = 0.15$, $P^{1}_\text{loss} = 0.1$ and $P^{1}_\text{stolen} = 0.05$ and a biometric credential Figure~\ref{fig:bio_atm_sourceafis_reg_scoring}.
...
$0.82$ and $0.78$ at the \EER point, and 
$0.84$ and $0.79$ at their respective optimal operating points.
}
...
For each target image, we define the final score methodolodgy as the maximum similarity across 6 probe templates, similarly to the previous simulation. 
\begin{table*}[t]
    \centering
...
\noindent
We introduce a formal model of authentication mechanisms that uses, for the first time, both biometric and atomic credentials. Our model characterizes each credential through four state probabilities, which allows us to analyze the success of different mechanism types. Using this model, we find that the optimal threshold of a biometric credential depends on the other credentials in the mechanism and differs from the heuristic threshold commonly used today. We then prove that there exists a mechanism uses an asymptotically separable biometric credential and~$k$ atomic credentials, with greater success probability than a mechanism uses the atomic credentials only. This result indicates that a mechanism designer will always benefit from adding a biometric credential into a mechanism uses atomic credentials only.

We quantitatively simulate different scenarios on public biometric datasets, 
including spoofing attacks. Our results show that optimizing the threshold significantly reduces the failure probability when a spoofing attack occurs. However, current biometric sensors often hide the threshold selection from mechanism designers and integrators, achieving a suboptimal mechanism success probability. We suggest that biometric sensor vendors should change their interface and let the integrators to set the thresholds of the biometric credentials. Harnessing our approach also provides the foundation for developing new authentication protocols and may enhace the security of digital and physical assets for millions of users around the world.
