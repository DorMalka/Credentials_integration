...
More suprisingly, integrating a biometric credential with an atomic one, improves a mechanism's success probability even when its atomic credential has arbitrary low fault probabilities and the biometric credential provides only weak separation between users and attackers. 
We quantitatively demonstrate the improvement with real biometric datasets. 

The direct implication is that biometric sensor vendors should allow setting thresholds, allowing better security in deployed authentication mechanisms.
\end{abstract}
