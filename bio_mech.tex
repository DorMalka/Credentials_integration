...
\begin{proposition}
    Let $c_2$ be a biometric credential with user distribution~$f_U(t)$ and attacker distribution~$f_A(t)$. Suppose that for every~$c_1$ an atomic credential with~$P^1_\text{safe} > 0$ and at least one of~$P^1_\text{leak},P^1_\text{loss} > 0$, there exists a threshold~$t$ and a mechanism~$\bar{M}$ which is an \emph{AND} or \emph{OR} composition using both~$c_1$ and~$c_2$, such that
    \begin{equation*}
...
\end{proof}
\noindent
We showed that~$\bar{M}$ achieves greater success probability than that of~$M$ if and only if the biometric credential is \emph{asymptotically separable} and the atomic credential has~$P^1_\text{safe} > 0$ and at least one of~$P^1_\text{leak},P^1_\text{loss} > 0$. Since biometric credentials often exhibit a more common behavior~\cite{Slob2005,anil2004}, we introduce an \emph{asymptotic dominancy} condition which captures this property. Intuitively, asymptotic dominancy means that one distribution function dominates the other in the tails of the distributions. This means that for very low score values~$s$ the attacker distribution dominates the user distribution, and vice versa for very high values of~$s$. We prove that a mechanism~$\bar{M}$ uses an \emph{asymptotically dominated} biometric credential and an atomic credential achieves greater success probability than that of a single atomic credential mechanism~$M$, and show it is a sufficient condition~(see Appendix~\ref{app:sufficient-condition}).
