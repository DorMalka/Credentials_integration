...
\begin{proposition}
    Let~$c_1$ be an atomic credential with~${P^1_\text{loss} = P^1_\text{leak} = 0}$, $c_2$ be a biometric credential with user distribution~$f_U(t)$ and attacker distribution~$f_A(t)$ and~$M$ be a mechanism using~$c_1$. Then, for every mechanism~$\bar{M}$ using~$c_1$,~$c_2$ it holds that
    \begin{equation*}
...
\begin{proof}
    ...
    \end{equation*}
    Each state probability of the biometric credential is strictly less than one; in particular,~$P^2_{\text{safe}}(t) < 1$ for every value of $t$. Therefore,~$P_{\text{success}}(t) \le P^1_\text{safe}$ and thus, adding a biometric credential does not produce a new mechanism~$\bar{M}$ whose success probability is greater than that of a single atomic credential mechanism.
\end{proof}
...
We now identify the conditions under which adding a biometric credential to a single atomic credential mechanism~$M$, can produce a new mechanism~$\bar{M}$ with a greater success probability.
...
\begin{proposition}\label{thm:suf_nec_cond_for_improve}
    Let~$c_1$ be an atomic credential with~$P^1_\text{safe} > 0$ and at least one of~$P^1_\text{leak},P^1_\text{loss} > 0$, $c_2$ be a biometric credential whose user distribution~$f_U(t)$ and attacker distribution~$f_A(t)$ are asymptotically separable and~$M$ be a mechanism using~$c_1$. Then, there exists a mechanism~$\bar{M}$ using~$c_1$,~$c_2$, such that
    \begin{equation*}
        P_{\mathrm{success}}^{\bar{M}} > P_{\mathrm{success}}^{M}
    \end{equation*}
\end{proposition}
...
\begin{proof}
    ...
    \contprobabilities
    A mechanism~$\bar{M}$ using~$c_1$ and~$c_2$ has three possible compositions: an \emph{AND mechanism} with success probability described by Equation~\ref{eqn:andsuccess}, an \emph{OR mechanism} with success probability described by Equation~\ref{eqn:orsuccess} and a mechanism using only the biometric credential with success probability described by Equation~\ref{eqn:safe_prob}. Thus, by placing the biometric credential in each configuration, we can express the success probability of~$\bar{M}$ as a function of the operating threshold~$t$
    \begin{align}
    &\ANDintegration \label{eqn:andineq} \\
    &\ORintegration \label{eqn:orineq} \\
    &P^{\textit{biometric}}_{\textit{success}}(t)=P^{2}_{\textit{safe}}(t)\label{eqn:bioeq}.
    \end{align}
    ...
    $P^{\text{AND}}_{\text{success}}(t) > P^1_{\text{safe}}$ or $P^{\text{OR}}_{\text{success}}(t) > P^1_{\text{safe}}$ or $P^{\textit{biometric}}_{\textit{success}}(t) > P^1_{\text{safe}}$.
    If there exists a threshold~$t$ such that~$P^{\textit{biometric}}_{\textit{success}}(t) > P^1_{\text{safe}}$ then the proof is trivial. Otherwise, we use both credentials to acheive dominance over using the atomic credential alone.
    ...
    \end{equation}
    Division by~$(1-\FAR(t))$ is valid under the imposed constraint that~$P^{2}_\textit{safe}>0$, which implies~$\FAR<1$ and~$\FRR<1$.
    ...
    = 0.
    \end{equation*}
    Equivalently,~$\forall \varepsilon > 0, \exists t$ such that either~$\frac{\FAR(t)}{1-\FRR(t)} < \varepsilon$ or~$\frac{\FRR(t)}{1-\FAR(t)} < \varepsilon$. Thus, we can choose $\varepsilon$ that equals the right hand side of either Equation~\ref{eqn:FRRineq} or Equation~\ref{eqn:FARineq} and by definition we know that a~$t$ satisfies the inequality exists. This existence of~$t$ does not break the constraint~$P^{2}_\textit{safe}>0$ since neither~$\FAR=1$ nor~$\FRR=1$ to satisfy the asymptotic separability condition. More precisely, if \[\inf_{t}\frac{\FAR(t)}{1-\FRR(t)} = 0,\] 
    then the \textsc{OR} mechanism composition is preferable, whereas if \[\inf_{t}\frac{\FRR(t)}{1-\FAR(t)} = 0,\] then the \textsc{AND} mechanism composition is preferable. Therefore, adding an asymptotically separable biometric credential to~$M$ produces a new mechanism~$\bar{M}$ with higher success probability.
\end{proof}

\begin{proposition}
    Let~$c_1$ be an atomic credential with~$P^1_\text{safe} > 0$ and at least one of~$P^1_\text{leak},P^1_\text{loss} > 0$, $c_2$ be a biometric credential with user distribution~$f_U(t)$ and attacker distribution~$f_A(t)$ and~$\bar{M}$ be a mechanism using both~$c_1$ and~$c_2$. Suppose there exists~$t$ such that
    \begin{equation*}
...
\begin{proof}
    ...
    \begin{equation*}
        ...
        \end{aligned}
    \end{equation*} 
    This contradicts the necessary condition that the success probability of~$\bar{M}$ is strictly greater than that of~$M$, as presented in both Equation~\ref{eqn:necessary term_1} and Equation~\ref{eqn:necessary term_2}. Therefore, the biometric credential must be asymptotically separable.
\end{proof}
We showed that \emph{asymptotic separability} of the user and attacker distributions is a necessary and sufficient condition for enhancing the success of a mechanism comprising an atomic credential with non zero probabilities~$P_\text{safe}$ and at least one of~$P_\text{leak}$ or~$P_\text{loss}$ by integrating it with a biometric credential.
