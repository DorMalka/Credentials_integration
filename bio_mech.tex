...
We formalize the authentication mechanism model comprising, for the first time, both biometric and atomic credentials. We analyze the success of different mechanisms' types and present a new match threshold by modeling each credential with four states probabilities. We show we can improve any mechanism success probability when it comprises an atomic credential only, by integrating it with an asymptotically separable biometric credential. 
We conduct experiments using real public biometric datasets, simulating different scenarios include spoofing attack, which became prevalant and easier on biometric credentials today. Our results show significant reduction of the failure probability when spoofing occurs.

Building new mechanisms protocols, based on the suggested match threshold~$(\S\ref{sec:4})$ is a promising direction for future work.

Our findings have important and impactful implications for the design of secure authentication mechanisms. We suggest letting the integrators to set the thresholds of the biometric credentials based on the other credentials they integrate into the mechanism. Harnessing our approach may affect millions of users around the world to enhace their security of digital and physical assets.
