...
We started by analyzing the case of a wallet comprised of 1 discrete credential and 1 continuous credential. We defined the discrete credential with a constant value of each one of the probabilities' states as: $ k_1, k_2,k_3, k_4 $ corrsponding respectively to each one of the states: ${\text{safe}}$, ${\text{loss}}$, ${\text{leak}}$ and ${\text{theft}}$. The continuous credential is defined by its PDFs for the user and the attacker, $P_U$ and $P_A$ respectively. Denote $s_i$ the state of credential $i$, we derived the probability of the credentails and the probability of success for the \textbf{AND} wallet as follows:

\[
\begin{aligned}
{P_{\text{safe1}}} &= 1 - \text{FAR}(T) - \text{FRR}(T) + \text{FAR}(T)\,\text{FRR}(T),\\
{P_{\text{safe2}}} &= k_1, \\
