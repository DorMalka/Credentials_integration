...
We will show that \emph{asymptotically dominancy} of user and attacker distribution functions is a sufficient and necessary condition for a continuous credential to improve the security of a wallet composed of discrete credentials.

\begin{theorem}
\label{thm:necessary condition}
Given a continuously distributed credential which is not asymptotically dominated, then for any wallet $w$ composed of one discrete credential and success function $P^{\text{discrete}}_{\text{success}}$, augmenting the wallet with this continuous credential does not improve its achievable security.
\end{theorem}
\begin{proof}
We prove this by contradiction. Assume for the sake of contradiction that the credential satisfies the theorem while the asymptotic domination condition does not hold. Therfore, there exists a threshold $T$ such that either:
\[
...
\]
These inequalities holds for every values of $P^{1}_{\text{safe}}, P^{1}_{\text{loss}}, P^{1}_{\text{leak}}$ in the range of $(0,1)$. Thus, we can deduce that the following inequalities hold for every right hand constant $c > 0$.
Since the asymptotic domination condition does not hold, then either $\exists c>0 \;\text{s.t.}\; \forall T \;\exists s \ge T:\;$
\[p_a(s) > c\,p_u(s)
\]
Or $\exists c>0 \;\text{s.t.}\; \forall T \;\exists s \le T:\;$
\[p_u(s) > c\,p_a(s)
\]
Thus, we can deduce that either:
\[
\frac{\mathrm{FRR}(T)}{1-\mathrm{FAR}(T)} = \frac{\int_{-\infty}^{T} p_u(s) ds}{\int_{-\infty}^{T} p_a(s) ds} \ge \frac{1}{c}
\]
...
\[ \begin{aligned}
P^{2}_{\text{safe}}(T) &= 1 - \text{FAR}(T) - \text{FRR}(T) + \text{FAR}(T) \cdot \text{FRR}(T),\\
P^{2}_{\text{loss}}(T) &= \text{FRR}(T)\,\bigl(1 - \text{FAR}(T)\bigr),\\
P^{2}_{\text{leak}}(T) &= \text{FAR}(T)\,\bigl(1 - \text{FRR}(T)\bigr),\\
P^{2}_{\text{theft}}(T) &= \text{FAR}(T) \cdot \text{FRR}(T).
\end{aligned}
\]
...
We can deduce that $P^{\text{AND}}_{\text{success}}(T) > P^1_{\text{safe}}$ if and only if:
\[
...
\begin{equation}
    ...
    \frac{\mathrm{FAR}(T)}{1-\mathrm{FRR}(T)} < \frac{P^{1}_{\text{loss}}}{P^{1}_{\text{safe}}+P^{1}_{\text{loss}}}
\end{equation}
For small values of $T$ in the interval of $(-\infty,T]$, we can look at Equation~\ref{eqn:FRRineq}. 
\[
...
\]
We show that either $\frac{\mathrm{FAR}(T)}{1-\mathrm{FRR}(T)} < \varepsilon$ or $\frac{\mathrm{FRR}(T)}{1-\mathrm{FAR}(T)} < \varepsilon$ for any $\varepsilon > 0$.
\end{proof}

This proof is not trivial, as the integration of another discrete credential to a discrete credentail based wallet, does not always enhance its security. For example, if the wallet is comprised of 1 discrete credential with high safe values and low leak values, adding a discrete credential with low safe values and high leak values would decrease the overall security. Thus, Theorem~\ref{thm:continuous-improves-security} shows that by using a separable continuous credential, we can always improve the security of any discrete credential based wallet.
