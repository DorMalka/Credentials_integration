...
For any wallet $w$ composed of one discrete credential and success function $P^{\text{discrete}}_{\text{success}}$, there exists $T'$ such that augmenting the wallet with a continuously distributed credential improves its achievable security. That is,
\[
P^{\text{integrated}}_{\text{success}}(T')
\ge
...
Our problem is divided into two cases:

a. $a_1<u_1$ : In this case, there exists an operating threshold $T'$ such that $a_1 < T' \le u_1$. In this interval, the FAR is strictly less than 1 and the FRR is equal to 0. Thus, we can choose using an \textbf{AND} wallet, represented by the success function:
\[
P^{\text{AND}}_{\text{success}}(T')
=
P^{1}_{\text{safe}}
...
P^{\text{AND}}_{\text{success}}(T).
\]
Since we assumed that $P^{1}_{\text{leak}}\ge0$ and $\mathrm{FAR}(T')\le1$, it follows that:
\[P^{\text{AND}}_{\text{success}}(T') \ge
P^{1}_{\text{safe}} = P^{\text{discrete}}_{\text{success}}
\]

b. $a_1 \ge u_1$ : In this case, there exists an operating threshold $T'$ such that $a_2 < T' < u_2$. In this interval, the FAR is equal to 0 and the FRR is strictly less than 1. Thus, we can choose using an \textbf{OR} wallet, represented by the success function:
\[
P^{\text{OR}}_{\text{success}}(T') =
P^{1}_{\text{safe}} +
P^{1}_{\text{loss}}\bigl(1-\mathrm{FRR}(T')\bigr)
\;\le\; \max_{T}\,P^{\text{OR}}_{\text{success}}(T).
\]
Since we assumed that $P^{1}_{\text{loss}}\ge0$ and $\mathrm{FRR}(T')\le1$, it follows that:
\[P^{\text{OR}}_{\text{success}}(T') \ge 
P^{1}_{\text{safe}} = P^{\text{discrete}}_{\text{success}}
\]
NOTE: Equalitiy is achieved only when $P^{1}_{\text{leak}}=0$ in case a. or $P^{1}_{\text{loss}}=0$ in case b., or $a_1 = u_1$ and $a_2 = u_2$ - which implies identical user and attacker distributions, making the continuous credential ineffective.
TODO
\end{proof}
