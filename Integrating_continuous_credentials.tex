...
In this work, we study the integration and usage of continuously distributed credentials in context of the wallet model. We looked for a dynamic and optimal operating point that yields higher probability of success, addressing the challanges of spoofing attacks. First, we defined the authentication model $(\S\ref{sec:3})$ built on a set of credentials and interacts with both user and attackers trying to identify the user.
...
\subsection{2 Continuous Credentials}
%%%
%%%
...
\section{Characterizing Security Improvement}
We will now characterize the conditions under which integrating a continuous credential into a wallet composed of a single discrete credential can enhance its security. First, we show that for any wallet whose discrete credential characterized by non-zero probabilities $P_\text{safe} \text{ and } P_\text{theft}$ only, such integration cannot improve security $(\S\ref{sec:5.1})$. Next, we present a necessary and sufficient condition establishing that, for any wallet composed of a discrete credential with non zero probabilities $P_\text{safe}, P_\text{leak} \text{ or } P_\text{loss}$, integrating a continuous credential improves security for both finite and infinite distributions $(\S\ref{sec:5.2})$. Finally, we provide a more intuitive sufficient condition under which an infinite continuous credential enhances the security of a wallet based on a single discrete credential with non zero probabilities $P_\text{safe}, P_\text{leak} \text{ or } P_\text{loss}$ $(\S\ref{sec:5.3})$.
\subsection{No Security Gain in the Safe/Theft Case}\label{sec:5.1}
\begin{proposition}
...
\end{proof}
\subsection{Sufficient And Necessary Condition for Integrating Continuous Credentials}\label{sec:5.2}
...
\subsection{Intuitive Sufficient Condition}\label{sec:5.3}
...
\section{Measuremens}\label{sec:6}
