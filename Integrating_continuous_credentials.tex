...
\title{2CIA: Continuous Credentials Integration in Authentication Mechanisms}
...
\end{proposition}
\begin{proof}
    Consider a wallet $w$ composed of a single discrete credential with non-zero probabilities $P_\text{safe}$ and $P_\text{theft}$ only. In this case, integrating the wallet with a continuous credential lead to success probability described by Equation~\ref{eqn:andsuccess} and Equation~\ref{eqn:orineq}. Since $P_\text{leak} = P_\text{loss} = 0$, the success probability in both wallet cases reduces to:
...
is said to be \emph{asymptotically dominated on a set of positive measure} if:
\[
\forall c>0\;\exists T\;\text{and } \exists A\subseteq D
\ \text{with}\ \mu(A)>0
\ \text{s.t.}:\ 
p_a(x)\le c\,p_u(x)
\quad \forall x\in A\cap[T,\infty).
\]
\[\quad\text{or}\quad\] 
\[
\forall c>0\;\exists T\;\text{and } \exists A\subseteq D
...
\ \text{s.t.}:\ 
p_u(x)\le c\,p_a(x)
\quad \forall x\in A\cap(-\infty,T].
\]
\end{definition}

We will show that \emph{asymptotically dominancy} of user and attacker distribution functions is a sufficient and necessary condition for enhancing a discrete based wallet's security.

\begin{theorem}
\label{thm:necessary condition}
For any wallet $w$ composed of of a single discrete credential with success function $P^{\text{discrete}}_{\text{success}}$, if augmenting $w$ with a continuous credential improves its achievable security, then that continuous credential must be asymptotically dominated
\end{theorem}
\begin{proof}
Since the integration of continuously distributed credential improves a single discrete credential wallet, then there exists a threshold $T$ such that either:
\[
P^{\text{AND}}_{\text{success}}(T)
=
\bigl(1-\mathrm{FRR}(T)\bigr)
...
Or:
\[
P^{\text{OR}}_{\text{success}}(T)=\bigl(1-\mathrm{FAR}(T)\bigr)\Bigl(P^{1}_{\text{safe}}+P^{1}_{\text{loss}}\bigl(1-\mathrm{FRR}(T)\bigr)\Bigr) > P^{1}_{\text{safe}}
\]
Thus, we can deduce that either:
\[\frac{\mathrm{FRR}(T)}{1-\mathrm{FAR}(T)} < \frac{P^{1}_{\text{leak}}}{P^{1}_{\text{safe}}+P^{1}_{\text{leak}}}
\]
Or:
\[\frac{\mathrm{FAR}(T)}{1-\mathrm{FRR}(T)} < \frac{P^{1}_{\text{loss}}}{P^{1}_{\text{safe}}+P^{1}_{\text{loss}}}
\]
These inequalities holds for every values of $P^{1}_{\text{safe}}, P^{1}_{\text{loss}}, P^{1}_{\text{leak}}$ in the range of $(0,1)$. Thus, as the following inequalities hold for every right hand constant $c > 0$.
\[
\frac{\mathrm{FRR}(T)}{1-\mathrm{FAR}(T)} = \frac{\int_{-\infty}^{T} p_u(s) ds}{\int_{-\infty}^{T} p_a(s) ds} < c \cdot \frac{\int_{-\infty}^{T} p_a(s) ds}{\int_{-\infty}^{T} p_a(s) ds}
\]
Then, we can deduce that $\forall c>0 \;\exists T \;\text{s.t:}\;$
\[\int_{-\infty}^{T} p_u(s) ds < c \cdot \int_{-\infty}^{T} p_a(s) ds
\]
...
\contprobabilities
The success probability of the integrated wallet comprised of 2 credentials is described by Equation~\ref{eqn:andsuccess} and Equation~\ref{eqn:orsuccess}. Thus, we can express the success probability of the integrated wallet as a function of the operating threshold $T$:
\ANDintegration
\ORintegration
We need to show that there exists an operating threshold $T$ such that either 
$P^{\text{AND}}_{\text{success}}(T) > P^1_{\text{safe}}$ or $P^{\text{OR}}_{\text{success}}(T) > P^1_{\text{safe}}$.
...
For large values of $T$ in the interval of $[T,\infty,)$, we can look at Equation~\ref{eqn:FARineq}. 
\[
...
This proof is not trivial, as the integration of another discrete credential to a discrete credential based wallet, does not always enhance its security. For example, if the wallet is comprised of 1 discrete credential with high safe values and low leak values, adding a discrete credential with low safe values and high leak values would decrease the overall security. Thus, Theorem~\ref{thm:continuous-improves-security} shows that by using a separable continuous credential, we can always improve the security of any discrete credential based wallet.
\subsection{}\label{sec:5.4}
\section{Conclusion}
