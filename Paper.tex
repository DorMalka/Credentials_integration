...
\left\{
\begin{array}{ll}
    a \cdot x^2+b \cdot x+c & \text{for } \alpha_1 \leq x \leq \alpha_2 \\
    0 & \text{for } x > \alpha_2 || x < \alpha_1
    \end{array}
\right.
\]
\[
...
\left\{
\begin{array}{ll}
    d \cdot x^2+e \cdot x+f & \text{for } \beta_1 \leq x \leq \beta_2 \\
    0 & \text{for } x > \beta_2 || x < \beta_1
    \end{array}
\right.
\]

Where $a,b,c,d,e,f$ are the parabolic coefficients for the user and attacker respectively. $\alpha_1, \alpha_2$ are the user PDFs' roots and $\beta_1, \beta_2$ are the attacker PDFs' roots as shown in Figure 4.
\begin{figure}[H]  
    \centering
...
To reduce the freedom degrees of the parabolic functions while keeping it symbolic anlysis, we defined the coeefficients based on the roots and the assumption that the integral of each PDF between its roots is equal to 1. Thus, we defined the coefficients as follows:
\[
\begin{array}{ll}
...
\left\{
\begin{array}{ll}
     \frac{6}{(\alpha_1 - \alpha_2)^2} \cdot x^2 - \frac{6 \cdot (\alpha_1 + \alpha_2)}{(\alpha_1 - \alpha_2)^3} \cdot x + \frac{6 \cdot \alpha_1 \cdot \alpha_2}{(\alpha_1 - \alpha_2)^3} & \text{for } \alpha_1 \leq x \leq \alpha_2 \\
    0 & \text{for } x > \alpha_2 || x < \alpha_1
    \end{array}
\right.
\]
...
\left\{
\begin{array}{ll}
     \frac{6}{(\beta_1 - \beta_2)^2} \cdot x^2 - \frac{6 \cdot (\beta_1 + \beta_2)}{(\beta_1 - \beta_2)^3} \cdot x + \frac{6 \cdot \beta_1 \cdot \beta_2}{(\beta_1 - \beta_2)^3} & \text{for } \beta_1 \leq x \leq \beta_2 \\
    0 & \text{for } x > \beta_2 || x < \beta_1
    \end{array}
\right.
\]

We will find FAR and FRR as a function of the threshold $T$ and define the probability of success based on single credential wallet as shown in section 3.2:
\[\text{FAR}(T) = \int_{\beta_1}^{T} P_A(t) dt =
\left\{
\begin{array}{ll}
0 & \text{if } T \le \beta_1 \\
\frac{6 \left(\frac{T^3}{3}-\frac{\beta_1^3}{3}\right)}{(\beta_1-\beta_2)^3} -\frac{6 \left(\frac{T^2}{2}-\frac{\beta_1^2}{2}\right) (\beta_1+\beta_2)}{(\beta_1-\beta_2)^3}+\frac{6 \beta_1 \beta_2 (T-\beta_1)}{(\beta_1-\beta_2)^3} & \text{if } \beta_1 < T \le \beta_2 \\
1 & \text{if } T > \beta_2
\end{array}
\right.
\]
\[\text{FRR}(T) = \int_{T}^{\alpha_2} P_U(t) dt =
\left\{
\begin{array}{ll}
1 & \text{if } T \le \alpha_1 \\
\frac{6 \left(\frac{\alpha_2^3}{3}-\frac{T^3}{3}\right)}{(\alpha_1-\alpha_2)^3}-\frac{6 \left(\frac{\alpha_2^2}{2}-\frac{T^2}{2}\right) (\alpha_1+\alpha_2)}{(\alpha_1-\alpha_2)^3}+\frac{6 \alpha_1 \alpha_2 (\alpha_2-T)}{(\alpha_1-\alpha_2)^3}& \text{if } \alpha_1 < T \le \alpha_2 \\
0 & \text{if } T > \alpha_2
\end{array}
\right.
\]
