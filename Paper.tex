...
In the 2nd interval, we can notice that the only critical point is at $T = \alpha_1$. In addition, for every $T > \alpha_1$, the derivative is positive, thus the critical point is a local minimum. In the 4th interval, the only critical point is at $T = \beta_2$. For every $T < \beta_2$, the derivative is negative, thus there is no maximum point in this interval either.
We will examine the 3rd interval, where the derivative is a multiplication of a cubic polynomial and two linear polynomials. The linear polynomials have roots at $T = \alpha_1$ and $T = \beta_2$, which are outside the interval, thus we will focus on the cubic polynomial:
The solution of the cubic polynomial can be found by various methods. Using mathematica tool by Wolfram, we found that there is one real root and two complex roots(TODO: Appendix).

Thus, there is only one critical point in the 3rd interval. By examining the sign of the derivative before and after the critical point, we can determine it is a global maximum. 
To prove this critical point, that represents the optimal threshold $T_{\text{opt}}$, is different than the EER point, we calculated the EER point as follows:
