...
\right.
\]
We derivative $P_{\text{success}}$ and find the critical points of it in each interval and then, compare between them to find the global maxima that represents the optimal threshold $T_{\text{opt}}$. 
\[
...
We therefore turn to the third interval. In this region, the derivative is the product of a cubic polynomial and two linear factors. The linear factors vanish at $T=\alpha_{1}$ and $T=\beta_{2}$, both of which lie outside the third interval. Consequently, the existence of critical points within this interval depends solely on the cubic term. Solving this cubic (Appendix),reveals exactly one real root with the remaining two roots being complex.

By examining the sign of the derivative on both sides of the critical point, we verify that it constitutes a global maximum of $P_{\text{safe}}(T)$. We denote this maximizing point by $T_{\text{opt}}$.
...
We illustrated the probability of success for a credential parabolic distributed depends on the threshold $T$ in Figure 5.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{fig_success_parabola.pdf}
    \caption{Probability of success for a single credential wallet parabolic distributed}
\end{figure}
Moreover, we defined the FAR vs FRR curve for the parabolic distributed credential in Figure 6.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{fig_FARvFRR_parabola.pdf}
